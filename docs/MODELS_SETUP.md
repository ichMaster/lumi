...
- **Inner monologue / think box.** On most providers the hidden think-step (v1.9 inner monologue) is empty.
...
- **Reasoning models — `gpt-5.5`, the o-series** → the **Responses API**. This is the one OpenAI path where
  ...
  honored, and a **reasoning summary** populates the think-box (the same seam Opus uses — so the v1.9 inner
