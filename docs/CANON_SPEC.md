...
The canon becomes the base of a system prompt that later versions extend *around* it (memory v0.2, the emotion-output instruction v0.3, the daily mood block v0.8). Specific downstream features depend on specific canon sections — write them deliberately:
...
| Mood of the day / temperament (v0.8) — a daily baseline that colors tone | §4.3 Temperament baseline + §4.11 Natal data |
...
3. **Temperament baseline.** *Her resting disposition on a few dials — energy, warmth, playfulness, talkativeness — as words, not numbers. This is the baseline the v0.8 horoscope-temperament modulates day to day (ARCHITECTURE §Mood and temperament). Make her recognizably herself before any mood shift.*
...
11. **Natal data** (optional, for v0.8). *Birth date / time / place used to compute her horoscope-temperament natal chart (ARCHITECTURE §Mood and temperament). If undecided, leave a clear `TBD` — the temperament phase can stub it.*
