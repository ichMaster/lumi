...
v2.1 this is refactored into a server client calling the same contract.
"""
...
class LumiApp(App[None]):
    ...
    def on_mount(self) -> None:
        ...
        if cfg.scheduler:
            ...
            # The fast tick service (LUMI-168): ephemeral code handlers (v1.7 registers %update_state).
    ...
    async def on_chat_input_submitted(self, event: ChatInput.Submitted) -> None:
        ...
        if not text:
            prompt.focus()
            return
        # v1.2: run the turn as a WORKER so this submit handler returns immediately. An inline
        # `await` here would hold the app's message pump for the whole reply — blocking the
        # dispatch of your subsequent keystrokes, so typing wouldn't show while she answers.
        # Off (no buffer) the box is disabled anyway, so a worker vs inline is behavior-identical.
        self.run_worker(self._handle_input_line(text), exclusive=False)
    ...
    def register_tick(self, name: str, handler) -> None:
        ...
        callback (no `Thought`, no model call). The seam v1.7 uses for `%update_state`; a no-op if the
