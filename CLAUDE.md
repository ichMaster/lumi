...
- [specification/FACE_THEMES.md](specification/features/FACE_THEMES.md) — the v0.10 face wardrobe: **several variants per emotion** (random pick, no immediate repeat) + **themed outfit packs** (`faces/<theme>/<emotion>/…`) where the **daily mood (v0.6) picks the theme**; extends the v0.7 viewer + signal, with a `calm`/default-theme fallback. No contract change.
- [specification/CLOSENESS.md](specification/features/CLOSENESS.md) — the v0.9 **relationship level**: a per-user closeness value bucketed into **5 levels**, read from each message (warmth/vulnerability/playful raise; harm/manipulation lower — folded into the reply turn) + **time decay**; the level biases warmth/openness, **never competence**; `Closeness{user_id, value, level, last_ts}`, per-user and isolated; a `/closeness` command.
