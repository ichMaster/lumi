...
- [specification/FACE_THEMES.md](specification/features/FACE_THEMES.md) — the v0.8 face wardrobe: **several variants per emotion** (random pick, no immediate repeat) + **themed outfit packs** (`faces/<theme>/<emotion>/…`) where the **daily mood (v0.6) picks the theme**; extends the v0.7 viewer + signal, with a `calm`/default-theme fallback. No contract change.
...
- [specification/VOICE_LOCAL.md](specification/features/VOICE_LOCAL.md) — the v0.11 local voicer: a separate console app that voices replies via ElevenLabs, reading the core's `outbox.jsonl` and marking `spoken.jsonl` (web sibling v2.2; shares the `/voice` TTS adapter).
- [specification/DICTATION_LOCAL.md](specification/features/DICTATION_LOCAL.md) — the v0.12 local dictator (STT, mirror of the voicer): a separate console app that listens (TUI-toggled via `listen.flag`), recognizes Ukrainian, and writes your line to `inbox.jsonl` like the keyboard (web sibling v2.4; shares the `/voice` STT adapter).
...
- **Memory scopes & the isolation invariant.** Лілі has **per-user (relationship) memory** — session history, short summaries `{ user_id, session_id, summary, gist, ts }` (v0.9: two-tier — the last N conversations injected in **detail** (`summary`) + all conversations in the last D local days as **gists**), long-term facts `{ user_id, fact, meta, confidence, ts }` — and a separate **shared experience** layer (`SharedMemoryItem`, de-identified). **Hard rule:** a record written under user A is never readable in user B's context; only de-identified, consent-gated, unattributed items cross via the v2.3 cross-pollination pipeline. Pin this with a contract test.
...
- **One model to start, more later.** The model is **Claude Haiku (Anthropic)** from **v0.1**, behind a thin **`LLMClient`** seam the core depends on (mockable; never the SDK directly). **More models** — other Claude tiers, OpenAI, DeepSeek, MiniMax — become switchable in config from **v0.13** (each provider's key in `.env`; OpenAI+DeepSeek share an OpenAI-compatible adapter). Structured output is per-provider (Anthropic tool output; OpenAI/DeepSeek JSON-schema; MiniMax JSON); the v0.3 validation gate is the safety net. Never bind `core` to a specific SDK.

## Intended stack & layout (not yet created)

Python, **Anthropic SDK — Claude Haiku (v0.1); more models (Claude tiers / OpenAI / DeepSeek / MiniMax) switchable v0.13**, Textual (TUI), FastAPI (server, v1+), ElevenLabs (TTS; later STT). Planned top-level layout from ARCHITECTURE.md:

```
/core           # canon, per-user memory + shared experience, llm (thin LLMClient seam — Claude Haiku v0.1; more models v0.13), emotion field + validation, mood/temperament (model-based daily mood + resolution, v0.6), repository interface (user-scoped)
...
/voice          # later (v0.11+): shared TTS adapter + local voicer (v0.11); shared STT adapter + local dictator (v0.12); reused by web voice (v2.2) / web dictation (v2.4)
