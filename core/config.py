...
class Config:
    ...
    stream: bool = False
    # v1.5 (LUMI-192): queue the post-turn persist (closeness + appends + recall index) on a single
    ...
    facts_enabled: bool = True      # LUMI_FACTS master switch: on → inject the ## Про Віталія static
    #                                 facts section (the skill-curated identity-core); off → no section.
    facts_core_max: int = 0         # v0.36: identity-core injection cap (0 → all core facts)
    ...
    file_date_max_days: int = 366   # v0.32 list_files after/before range-span cap (days)
    ...
    thought_surface: bool = False  # v0.33 surface a running directive as a chat-log meta line (TUI)
...
def load_config(*, load_env: bool = True) -> Config:
    ...
    return Config(
        ...
        facts_enabled=(os.getenv("LUMI_FACTS") or "on").strip().lower() in _TRUTHY,  # master switch, on by default
        ...
        file_date_max_days=int(os.getenv("LUMI_FILE_DATE_MAX_DAYS") or 366),
