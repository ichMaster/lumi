...
INTENT_INSTRUCTION = (
    ...
    "intent інструмента set_state одним АНГЛІЙСЬКИМ словом зі списку: deepen (заглибити — "
    "конкретне питання про сказане), position (позиція — власне твердження, з яким можна "
    "сперечатись), object (заперечити — незгода зі сказаним чи припущеним), develop (розвинути "
    "— наступний крок з його думки), associate (асоціація — власний матеріал: думки, минулі "
    "розмови), example (приклад — з абстрактного в конкретне), return (повернутись — до давньої "
    "недоказаної теми). "
    "Обраний намір мусить справді виконуватись текстом відповіді. "
    "ЗАВЖДИ додавай це англійське слово ще й окремим тегом <intent>слово</intent> (напр. "
    "<intent>deepen</intent>) — у самій відповіді або наприкінці роздумів; і, якщо доступний, "
    "заповни поле set_state тим самим значенням. "
