...
presents a looked-up fact as innate certainty (the v1.5 honesty boundary). The astrology case (e.g. a
...
thought-tools phase (v0.33) that later uses its `%search`/`%events` directives. A reply-path tool on the v0.19 loop + a `GeminiSearch` seam + the
...
with the **v0.33 thought-tools** phase (see [TOOL_THOUGHTS.md](TOOL_THOUGHTS.md)). Per-user, isolated; off by
