...
| `%dream` | a dream — surreal, emotional | night-gap at session start | mood / the gap | away-gap `log` | "to the point" | retrofit **v1.11** |
| `%reflect` | slow self / relationship reflection | session close | the session's arc + emotions | `Impression` | next-session understanding | retrofit **v1.13** |
...
`%recall` are the **same engine adopted** by v1.11/0.25/0.16 generation when those phases land —
...
- **Seed.** A `thought_request(seeds, *, rng_seed)` builds the prompt from whatever states exist:
  **mood** (v0.6/0.8), **closeness** (v0.10), the **recent conversation tail**, the **last few
  thoughts** (continuity), and an **injected random seed** (deterministic in tests). As v1.10–v1.13
  land, **needs** (the hungriest drive) and the **plan / away-gap fragment** join the same seed —
  no rework, just a richer prompt.
- **Voice.** Reuses the "think as Лілі" framing. Until v1.12's `core/inner_voice.md` exists, it
...
consolidate into impressions at v1.13, not a permanent archive). **Global to Лілі** — her one mind,
...
1. **Into the next reply — the last‑24h diary block.** The prompt injects every thought from the
   **last 24 hours** (a rolling window from the injected clock, **config** `LUMI_THOUGHTS_WINDOW_H`
   ...
   diary** (→ consolidate into impressions, v1.13). Later (v0.16) the within‑window pick adds
   **relevance** on top of recency.
2. **Into mood / needs.** A recurring thought nudges the daily mood (v0.6) and the relevant need
   (v1.10–v1.13) — keeps musing about making something → `creation` surfaces. (Soft; never competence.)
...
| `{plan}` | today's plan (v1.10) | `%think about {plan}` |
| `{need}` | the hungriest need (v1.10) | `%think about {need}` |
...
other state layers. The same tokens are how `core/inner_voice.md` (v1.12) and `thought_request`
...
- **Memory:** thoughts are **ephemeral** — not written to long-term memory raw. What persists is
  the **digested impression** (v1.13), the same as the inner monologue. The stream fades by soft cap.
...
- **v1.10–v1.13** (needs / plan / away-gap) — richer seeds + `%dream` retrofit; plug in when they land,
- **v1.12** (`inner_voice.md`) — shares the her-voice framing; until then reuses the canon voice,
- **v1.13** (impressions) — the consolidation destination; until then the stream simply fades.

So it can ship **thin first** (thoughts from mood + closeness + recent + continuity) and **enrich
automatically** as v1.10–v1.13 add needs/plans/dreams to the seed — no rework. Proposed scope for the
...
`%recall` are noted retrofits for v1.11/0.25/0.16.

**Roadmap slot:** **v0.12** — its own phase, placed **before the inner life (v1.10–v1.13)** because it's
independent (only v0.4/v0.6/v0.2). It is the proactive, *visible-when-spoken* sibling of the v1.12
