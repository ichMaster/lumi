...
- **v0.20 — Index & search:** the `Embedder` + `VectorStore` seams, index/backfill, and the
  explicit `/recall <query>` search.
- **v0.21 — Automatic RAG:** the incoming message is the query; the most relevant past moments
...
| long-term: facts + impressions (v0.18) | durable understanding | not verbatim, not exhaustive |
| **semantic recall (v0.20–v0.21)** | **every message embedded → searched by meaning** | **exact recall of anything, anytime** |
...
## v0.20 — `/recall <query>` (search on request)
...
## v0.21 — automatic RAG in the turn
...
`sqlite-vec`), both behind the seams. Added to `pyproject.toml` when v0.20 is built.

## Mapping to the roadmap

**v0.20 + v0.21 — Semantic recall**, right after the emotional-memory layer (v0.18–v0.19) — the
