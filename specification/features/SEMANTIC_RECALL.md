...
- **v0.17 — Index & search:** the `Embedder` + `VectorStore` seams, index/backfill, and the
  explicit `/recall <query>` search.
- **v0.18 — Automatic RAG:** the incoming message is the query; the most relevant past moments
...
| long-term: facts + impressions (v0.15) | durable understanding | not verbatim, not exhaustive |
| **semantic recall (v0.17–v0.18)** | **every message embedded → searched by meaning** | **exact recall of anything, anytime** |
...
## v0.17 — `/recall <query>` (search on request)
...
## v0.18 — automatic RAG in the turn
...
`sqlite-vec`), both behind the seams. Added to `pyproject.toml` when v0.17 is built.

## Mapping to the roadmap

**v0.17 + v0.18 — Semantic recall**, right after the emotional-memory layer (v0.15–v0.16) — the
