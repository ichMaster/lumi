...
This is the **sixth** personality layer and it is **distinct** from the others: inner life (v1.8)
holds thoughts *between* sessions; emotional memory (v1.11) holds thoughts *after* a session; this
is thinking *in the moment* — the hidden step inside the reply turn. It is the place where all the
existing states converge into a decision *how to speak*. Scheduled at **v1.10** (right after the
...
2. **The state blocks in context**, so the monologue is concrete, not abstract: the already-built
   **mood** (v0.6/0.8), **closeness** (v0.10), **needs** (v1.6–v1.9), and — when they exist — any
...
- **Do NOT write think into long-term memory by default.** Thoughts are ephemeral, like a person's.
  What persists is the **digested impression** via emotional memory (v1.11), not the raw monologue.
...
**v1.10 — Inner monologue (think-phase in her voice)**, once the states it weighs exist:
**v0.6/0.8** (mood), **v0.10** (closeness), **v1.6–v1.9** (needs + plans), plus later **self-regard**
