...
3. **Two cadences.** A normal tick (`LUMI_SCHED_TICK_MS`) evaluates `due()` for the authored schedule
   ...
   flows only while the TUI runs — state saved on close, resumed on start, v1.6). So the v0.4 nudge + the v0.12 `%think` idle trigger **fold in** as
...
there the same `due()` + `run_directive` + the v1.6/v1.8 `update(state, now)` simply move into the server loop,
unchanged.
