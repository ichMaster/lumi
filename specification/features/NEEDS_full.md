...
Лілі's daily temperament (v0.6) and inner life (v0.17) describe **what she feels** and **what she
...
v0.8), a **bias on the inner-life plan + free-slot choice** (v0.17), and they **close from what
actually happened** in her generated days (v0.18).

**Lands across v0.17–18** (woven into the inner life, not a phase of its own — see §13). It builds
...
`connection`; and the global **inner-life store** + away-gap generation (v0.17/0.18), where the
...
less lonely; after a week of silence `connection` decays with the rest. (Lands in v0.17 with the
...
1. **Daily mood call (v0.6), beside biorhythms (v0.8) — [v0.17].** Current levels — especially the
...
2. **Inner-life plan + free-slot choice — [v0.17 tilt, v0.18 fill].** The hungriest deficit **tilts**
   today's plan (1–2 items) and the free-slot activity toward what serves it; a served slot then
   replenishes that need (closing the loop — the fill + replenish land with the away-gap, v0.18).
3. **Idle nudge (v0.4) — [v0.17].** An unprompted nudge can be voiced by the dominant need ("I need
...
Needs are **woven into the inner-life phases v0.17–18** (not a phase of their own), because the loop
(needs → plan → reality → close) *is* the inner life. The split follows §9's two halves:

**v0.17 — Inner life I (the drives exist & pull):**
...
**v0.18 — Inner life II (the drives close from reality):**
...
- the **threshold-5 per-day generation** rule (§7), refining the v0.18 gap-fill.
...
v0.17 inner-life store/away-gap it rides in. An experiment for daily variation from the inside out
