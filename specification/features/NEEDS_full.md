...
live on the v0.43 **tick** (evolving only while the TUI runs — §6); they are **another computed
...
life — see §13). It builds on seams that already exist: the v0.43 **tick service** (the code-handler
...
**Her time flows only while the TUI runs.** The levels evolve on the v0.43 **fast tick**:
...
- the pure **`evolve`** — fractional-Δt **decay + drift** (§6's three rules: fractional Δt,
  split-invariance, evolve-on-read/-before-mutate), run by the v0.43 **fast tick** (`%update_state`
...
Depends on **v0.43** (the tick service the levels ride), **v0.6** (the mood it feeds), **v0.8**
