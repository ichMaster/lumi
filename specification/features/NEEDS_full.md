...
Лілі's daily temperament (v0.6) and inner life (v1.8) describe **what she feels** and **what she
...
input to the daily mood call** (like biorhythms, v0.8), can **fire an act on a threshold** (v1.7),
**bias the inner-life plan + free-slot choice** (v1.8), and **close from what actually happened**
in her generated days (v1.9).

**Lands across v1.6–v1.9** (the drives open v1 as their own two phases, then weave into the inner
...
global **inner-life store** + away-gap generation (v1.8/v1.9), where the activity-based closing
happens.
...
less lonely; over TUI-alive silence `connection` decays with the rest. (Lands in v1.6 with the
...
into a handler registry nothing ever populated. v1.6 registers its **first actual handlers**, starting
...
with the away-gap phase (v1.9): its retroactive life brings the gap's **decay together with the
...
## 9. The algorithm (the full v1.9 away-gap pass)

> In v1.6–v1.7 only the **tick loop** runs: `evolve` (fractional-Δt decay + drift, §6) → threshold
> check → maybe fire. The pass below is the **v1.9 away-gap** algorithm — the one place gap time
...
1. **Daily mood call (v0.6), beside biorhythms (v0.8) — [v1.6].** Current levels — especially the
...
2. **Inner-life plan + free-slot choice — [v1.8 tilt, v1.9 fill].** The hungriest deficit **tilts**
   today's plan (1–2 items) and the free-slot activity toward what serves it; a served slot then
   replenishes that need (closing the loop — the fill + replenish land with the away-gap, v1.9).
3. **Threshold acts — [v1.7].** A drive crossing its authored threshold fires its directive on the
...
Needs open v1 as **two phases of their own** (v1.6–v1.7), then weave into the inner-life phases
(v1.8–v1.9), because the loop (needs → plan → reality → close) *is* the inner life:

**v1.6 — Needs I (the drives — tick-driven levels that pull her):**
- the global **`Needs{levels, last_ts}` store** + authored `core/needs.md` (the 6 drives:
  decay/weight/satisfied-by/deficit voice + the **threshold & cooldown** fields v1.7 uses);
...
**v1.8 — Inner life I (plans & state):** needs **tilt today's plan** (1–2 items — §5.1 / §10.2) —
the tilt lands here, where a plan first exists.

**v1.9 — Inner life II (the drives close from reality):**
...
clock), **v0.2** (the Repository); the v1.8 inner-life store / v1.9 away-gap it weaves into. An
