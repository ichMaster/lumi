...
The turn is **synchronous**: give a prompt — wait for the new canvas — next turn. No asynchrony is needed (one step is one generation, seconds) — so the canvas does **not** use the v6.2 async-jobs mechanism, even though it uses the v6.3 `image` generator.
...
- the `image` generator (v6.3) — generation / (later) editing of the canvas;
- **Anthropic vision** (§Vision, v6.1) — so Лілі perceives the current canvas (the image goes into the context of her reply, not a separate call).

A plain "show Лілі a photo" is a free bonus of the same vision capability (v6.1). Finished canvases go into the [gallery](GALLERY_MCP.md).
...
- Off by default, per-user (admin panel, v3.5); limits (rate, cost cap); logging.
...
**v6.4 — Co-creation canvas**, after `image` (v6.3, which it relies on) and vision (v6.1). The turn is synchronous (no async). We start with the layer-by-layer mode and Лілі's first move; inpainting comes later. Depends on: v6.1 (gallery + vision), v6.3 (the `image` generator).
</content>
