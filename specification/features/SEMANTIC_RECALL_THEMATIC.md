...
- **Retrieve topic-first.** Top-`K` is taken **preferring records that share an active topic**, then
  topped up from the rest so a turn is never starved — then the v0.17/v0.23 expansion + injection runs
  unchanged.
...
- **`topics`** — the tuple of topic names this record (message, or chunk under v0.23) is assigned to,
  from the taxonomy; possibly empty (a message that matches no topic above the floor). 0-based, order by
  descending assignment score.

Everything else (`user_id`, `msg_id`, `vector`, `text`, `ts`, `role`, and v0.23's `parent_msg_id` /
`chunk_index`) is unchanged.

## Classification at index time (local, no LLM)

- **Assign on write / backfill:** for each message (or chunk), cosine the record's **own vector**
  against every topic centroid; assign the topics scoring **≥ `topic_floor`**, capped at
  `topic_max` labels. A record matching nothing stays **untagged** (it can still be recalled by raw
...
- **No re-embedding to re-tag.** Topic labels are derived from the **already-stored vectors**, so
  ...
  to the embedder**. The taxonomy version joins the vectors staleness tag (→
  `model@embed_max_chars@chunk_chars@topics_vN`) so the labels rebuild when the taxonomy changes, the
  same way a model/cap/chunk change re-indexes — but here only the labels recompute, not the vectors.
...
1. **Local pick (always, no LLM):** cosine the **query** embedding against the topic centroids → topics
   ≥ `topic_floor`, capped at `topic_max`.
...
Just as she emits her own emotion, Лілі emits the topics she senses as part of the reply turn — the same
**additive `RelationRead`** mechanism v0.10 uses for warmth/closeness. The core **validates** the
emitted topics against the taxonomy (unknown → dropped) and folds them into the carried-forward active
set. Because the RAG block for a turn is assembled **before** the model replies, her emitted topics take
...
1. **Search** — embed the query → top-`K` over this user's vectors → relevance floor (unchanged).
2. **Topic routing (new):** partition the floor-passing hits into **on-topic** (shares an active topic)
   ...
   no-op → identical to v0.17/v0.23.
3. **Expansion + injection (v0.17 / v0.23):** the chosen hits expand to their dialogue thread (and, under
   v0.23, their passage) and inject into the same `# Релевантні моменти минулого` block — unchanged.
...
## Composition with chunking (v0.23)
...
**v0.24 — Semantic recall IV: thematic recall (topic routing)**, a refinement of the recall line
(v0.16 index + v0.17 auto-RAG/context-expansion), placed after v0.23. Adds a topic label to each
`VectorRecord`, a local embedding classifier (index-time tagging + per-turn active-topic pick), a router
in front of the v0.17/v0.23 selection, and Лілі's topic `RelationRead`. Depends on **v0.16** (the index
+ seams) and **v0.17** (auto-RAG + context expansion); **independent of v0.23** (composes with chunks).
Reuses the v0.10 `RelationRead` emit pattern and the v0.6/v0.10 read-state command pattern. Per-user,
isolated; local-or-cloud per the configured embedder. **Off by default** (`LUMI_RAG_TOPIC`) → behaves
exactly like v0.17/v0.23 (one undifferentiated pool).
