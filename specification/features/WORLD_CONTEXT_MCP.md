...
- **Knowledge** — structured and fresh facts that complement the v5.2 web search.
...
Each is an MCP tool reached through the v5.2 MCP client; the bounded tool loop feeds results back as tool messages (degraded reply on error/timeout, never a hang).

## Boundaries and safety

- **Off by default;** a per-user toggle (default false, in the admin panel, v3.5) — like `web_search`. May be one flag for the whole layer or per source.
...
Lands as **v5.3 — World context & knowledge (MCP)**, right after web search (v5.2), reusing the v5.2 MCP client + tool loop. Build order within the phase: **weather + time/moon first** (the cheap ambient sources that color the day), then **wiki/news** as a complement to web search. See ROADMAP §v5.3.
</content>
