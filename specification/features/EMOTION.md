...
- **Web portrait + caption (v4.1):** emotion → the same portrait in the web interface, plus a short descriptive caption (§6).
- **Animation (v5.1):** the portrait comes alive (transitions, idle motion, lip-sync to voice).
...
- Per-frame model calls. Emotion is decided once per turn with the reply; animation/lip-sync (v5) run locally off that single state plus the TTS audio.
...
- `intensity` — float 0.0–1.0. Scales presentation: emoji variant/emphasis (v0.5),
  portrait intensity variant (v4.1), animation amplitude and idle motion (v5).
  Required.
- `ttl_ms` — int, optional (default 8000). After this with no new turn, an
  animated renderer (v5) relaxes toward `calm`. Ignored by the log/emoji/static tiers.
- `speaking` — bool, optional (default false). Set by the renderer (not the model)
  while voice (v4.2) is playing, to drive lip-sync (v5). Not part of the model's output.
...
| emotion      | reads as                                  | emoji (v0.5) | portrait key (v4.1) |
|--------------|-------------------------------------------|------------|-------------------|
...
the portrait-key column is the v4.1 asset manifest key (§7).
...
class IEmotionRenderer(Protocol):
    ...
    def set_speaking(self, speaking: bool) -> None: ...     # v4.2+ voice → lip-sync (v5)
    def tick(self, dt_ms: int) -> None: ...                 # v5 idle loop: transitions, micro-motion
```
...
- **`ImageRenderer` (v4.1)** — resolves `emotion`(+`intensity`)→a portrait asset (§7, the **same pack** as the v0.7 viewer) and swaps the web portrait panel; also shows the §6 caption.
- **`AnimationRenderer` (v5.1)** — `render` sets a target expression and crossfades; `tick` runs the idle loop (blink, breathe, micro gaze-drift); `set_speaking` + the TTS amplitude envelope drive mouth lip-sync.

Only the renderer changes between versions. The `EmotionState` and the enum are constant.

## 6. Emoji mapping (v0.5) and mood caption (v4.1)
...
**Mood caption (v4.1).** Alongside the web portrait, a short evocative **caption**
...
## 7. Image asset manifest (v0.7 / v4.1)
...
when no variant matches. Full PNG quality, no palette limits (ROADMAP v4.1 DoD).
...
## 9. Voice delivery (v4.2, optional)
...
- **v4.1 — web portrait + caption.** `ImageRenderer` + the same asset manifest (§7) in the browser, plus the §6 mood caption. No contract change.
- **v4.2 — voice.** Optional emotion-biased TTS delivery (§9); renderer sets `speaking`.
- **v5.1 — animation.** `AnimationRenderer` (§5): transitions, idle loop, lip-sync. The same `EmotionState` drives it.
...
- `web/` (v3.4+) — `ImageRenderer` + the mood caption, the portrait panel, and the same asset pack (`lili_v1`); `AnimationRenderer` (v5).
</content>
