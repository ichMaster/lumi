...
Boundaries for the optional **web search** capability (ROADMAP §v5.2; ARCHITECTURE §MCP tools, §Security, auth, and access). It lets Лілі look things up on the open internet — **within strict bounds** — through an MCP tool, and is **off by default**. Adapted from the Pyramid project's web-search design.
...
A networked MCP service (HTTP/SSE), called by the model like any other tool during the v5.2 tool loop. The server hosts the MCP client; the core's model turn issues the calls.
...
- Per-user toggle `User.web_search`, **default `false`**, edited in the admin panel (v3.5).
...
- Runs inside the v5.2 MCP **tool loop** (ARCHITECTURE §MCP tools): bounded iterations, tool results fed back as tool messages; on tool error/timeout the turn returns a **degraded reply** (answer from model knowledge + a note), never fails the turn. The emotion field `{reply, emotion, intensity}` is produced exactly as on any other turn.
