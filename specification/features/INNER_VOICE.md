...
- the v1.12 inner monologue ([INNER_MONOLOGUE.md](INNER_MONOLOGUE.md) / [ROADMAP.md](../ROADMAP.md) §v1.12) —
...
| Voice **volume** shifts by needs / social-battery / self-regard | those state stores (v1.10–v1.11+) — **not built** | **deferred** (seam) |
...
- **Logged** to the v0.3 logged tier; **never written to long-term memory** — the raw monologue is
  ephemeral (only a later v1.13 *impression* persists).
...
- **Voice volume from state (v1.10–v1.11+).** `creation` need → Імпульс louder; high `oversaturation` / low
...
- **Not required for this version:** v1.10–v1.11 (needs + plans), Автоверс, the `maturity` subsystem — all
