...
- the v1.3 inner monologue ([INNER_MONOLOGUE.md](INNER_MONOLOGUE.md) / [ROADMAP.md](../ROADMAP.md) §v1.3) —
...
| Voice **volume** shifts by needs / social-battery / self-regard | those state stores (v1.1–v1.2+) — **not built** | **deferred** (seam) |
...
- **Logged** to the v0.3 logged tier; **never written to long-term memory** — the raw monologue is
  ephemeral (only a later v1.4 *impression* persists).
...
- **Voice volume from state (v1.1–v1.2+).** `creation` need → Імпульс louder; high `oversaturation` / low
...
- **Not required for this version:** v1.1–v1.2 (needs + plans), Автоверс, the `maturity` subsystem — all
