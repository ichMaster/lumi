...
- the v1.9 inner monologue ([INNER_MONOLOGUE.md](INNER_MONOLOGUE.md) / [ROADMAP.md](../ROADMAP.md) §v1.9) —
...
| Voice **volume** shifts by needs / social-battery / self-regard | those state stores (v1.7–v1.8+) — **not built** | **deferred** (seam) |
...
- **Logged** to the v0.3 logged tier; **never written to long-term memory** — the raw monologue is
  ephemeral (only a later v1.10 *impression* persists).
...
- **Voice volume from state (v1.7–v1.8+).** `creation` need → Імпульс louder; high `oversaturation` / low
...
- **Not required for this version:** v1.7–v1.8 (needs + plans), Автоверс, the `maturity` subsystem — all
