...
- the v1.11 inner monologue ([INNER_MONOLOGUE.md](INNER_MONOLOGUE.md) / [ROADMAP.md](../ROADMAP.md) §v1.11) —
...
| Voice **volume** shifts by needs / social-battery / self-regard | those state stores (v1.9–v1.10+) — **not built** | **deferred** (seam) |
...
- **Logged** to the v0.3 logged tier; **never written to long-term memory** — the raw monologue is
  ephemeral (only a later v1.12 *impression* persists).
...
- **Voice volume from state (v1.9–v1.10+).** `creation` need → Імпульс louder; high `oversaturation` / low
...
- **Not required for this version:** v1.9–v1.10 (needs + plans), Автоверс, the `maturity` subsystem — all
