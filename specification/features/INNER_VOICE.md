...
- the v2.4 inner monologue ([INNER_MONOLOGUE.md](INNER_MONOLOGUE.md) / [ROADMAP.md](../ROADMAP.md) §v2.4) —
...
| Voice **volume** shifts by needs / social-battery / self-regard | those state stores (v2.2–v2.3+) — **not built** | **deferred** (seam) |
...
- **Logged** to the v0.3 logged tier; **never written to long-term memory** — the raw monologue is
  ephemeral (only a later v2.5 *impression* persists).
...
- **Voice volume from state (v2.2–v2.3+).** `creation` need → Імпульс louder; high `oversaturation` / low
...
- **Not required for this version:** v2.2–v2.3 (needs + plans), Автоверс, the `maturity` subsystem — all
