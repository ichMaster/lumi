...
- **On game end**, the *experience* (not the move log) may distil into one `Impression` via the existing
  emotional-memory path (v1.10) — "a long, sharp game with you; she liked the endgame" — so the game enters
...
maintainer prefers; it has no ordering dependency on those. Optionally composes with **v1.10** (emotional
