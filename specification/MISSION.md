...
- **One model to start, more later.** Лілі runs on **Claude Haiku (Anthropic)** from v0.1 — the only model to begin with; **more models** (other Claude tiers, OpenAI, DeepSeek, MiniMax) become switchable in config from v0.21. The core depends on a thin `LLMClient` seam, never a concrete SDK.
...
- **Emotion viewer** — a local desktop window (v0.7) showing a portrait of Лілі for her current emotion, from a `faces/` asset pack — the local-stage image face before the web (v2.1). See [EMOTION_VIEWER.md](features/EMOTION_VIEWER.md).
...
- **Voice output** — synthesis of the reply's voice via ElevenLabs: a **local voicer** console app first (v0.19, see [VOICE_LOCAL.md](features/VOICE_LOCAL.md)), then server-side in the web (v2.2). Same TTS adapter.
- **Dictation** — Ukrainian voice input (STT): a **local dictator** console app first (v0.20, the mirror of the voicer — see [DICTATION_LOCAL.md](features/DICTATION_LOCAL.md)), then server-side in the web (v2.4). Same STT adapter.
...
- **World context** — optional, off-by-default, passive MCP tools (weather, time, holidays, moon, wiki, news) that give Лілі an ambient sense of the day and structured facts; the ambient sources feed her v0.6 mood (alongside the horoscope), coloring tone, never competence (see [WORLD_CONTEXT_MCP.md](features/WORLD_CONTEXT_MCP.md)). v3.3.
