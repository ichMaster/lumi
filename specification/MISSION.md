...
- **Voice output** — synthesis of the reply's voice via ElevenLabs: a **local voicer** console app first (v0.16, see [VOICE_LOCAL.md](features/VOICE_LOCAL.md)), then server-side in the web (v2.2). Same TTS adapter.
- **Dictation** — Ukrainian voice input (STT): a **local dictator** console app first (v0.17, the mirror of the voicer — see [DICTATION_LOCAL.md](features/DICTATION_LOCAL.md)), then server-side in the web (v2.4). Same STT adapter.
