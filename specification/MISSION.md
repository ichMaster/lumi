...
- **One model to start, more later.** Лілі runs on **Claude Haiku (Anthropic)** from v0.1 — the only model to begin with; **more models** (other Claude tiers, OpenAI, DeepSeek, MiniMax) become switchable in config from v0.16. The core depends on a thin `LLMClient` seam, never a concrete SDK.
...
- **Dictation** — Ukrainian voice input (STT): a **local dictator** console app first (v0.15, the mirror of the voicer — see [DICTATION_LOCAL.md](features/DICTATION_LOCAL.md)), then server-side in the web (v2.4). Same STT adapter.
