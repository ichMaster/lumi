...
### v1.6 — Model roles: register-routed replies (talking / thinking / emotional)

**Goal:** the reply model stops being one fixed choice — each turn answers in one of three **registers**: **`talking`** (the everyday voice, fast/cheap), **`thinking`** (hard analytical tasks, extended reasoning), **`emotional`** (loaded moments, the fullest persona) — picked automatically by a staged **classifier** on a fourth **`classifier`** role (a tiny model), with **stickiness** so a register holds through its moment. So "розкажи як влаштований кеш" gets the thinking engine, "мені сьогодні важко" gets the warm one, and "дякую, добраніч" never pays frontier prices.

**The hybrid flow (waste-free by default):** a **lexical stage** runs first — code, no LLM, ~0 ms: message length, an authored closers/greetings list, distress markers, code blocks / multi-part questions, an explicit override, the active hold. A clear *talk* (short messages always land here) → fire **talking only**; strong thinking/emotional markers → fire **that model only**; unsure → fire **talking + the classifier in parallel** — one structured call returning `{label, confidence}` (one call, not two questions; below `LUMI_ROLE_CONFIDENCE` → talk). An escalation re-sends the same message to the chosen model and discards the talking draft; no escalation → the talking reply is already in hand (zero added latency). **Stickiness:** an escalation holds its register for `LUMI_ROLE_COOLDOWN` turns; **short messages bypass the hold** (classified fresh, usually answered by talking) but the countdown **keeps running**; a new escalation re-targets + resets; then back to talking. **Routing reads the user's message, never her mood — never competence.** The roles live in the v0.41 profiles (each `[profiles.*]` block gains `talking`/`thinking`/`emotional`/`classifier` — provider-homogeneous); the v0.40 op-tiers and `/model-set` compose unchanged; a manual `/model` pins the reply and suspends role-routing for the session (an explicit choice wins). Off (`LUMI_MODEL_ROLES`, default) → today's single reply model, **byte-identical**. Full design: [MODEL_ROLES.md](features/MODEL_ROLES.md). This phase is also [LATENCY.md](features/LATENCY.md) **LAT-3**, with latency as a driving DoD (casual turn ≤ 4 s full), and it folds in **S4-interim** — `LUMI_EFFORT=low` for the `talking` register + the **trimmed talking-tier `inner_voice.md`** (the short inner voice the fast register uses; the `thinking`/`emotional` registers keep the full inner voice). Depends on: v0.41 (profiles + `switch_profile`), v0.40 (`_model_for` — untouched), v0.3 (the emotion gate).

**Tasks:**
- **Roles in the profiles:** `ModelProfile` gains `talking` / `thinking` / `emotional` / `classifier` (additive, defaulted from `reply` → off-safe); `core/models.toml` ships the authored sets (anthropic: sonnet-5 / opus+thinking / opus / haiku).
- **The lexical stage** (`core/roles.py` + an authored `core/roles.md`): closers/greetings, distress + thinking markers, length/code-block/multi-question heuristics, the hold state → `talk | think | emotional | unsure` (table-driven, no LLM).
- **The classifier call:** one structured request to the `classifier` role — the message + the last few labels, **no memory/personal context** — → `{label, confidence}`; floor → talk; any failure → talk (never blocks a turn).
- **Hybrid firing** in the reply path: lexical-clear → one request; unsure → talking + classifier in parallel, escalate on the verdict (discard the draft); the register's model is the reply call's `model` (the tool-loop follows it, v0.40-style).
- **Stickiness:** the hold counter (per conversation, in-memory), short-message bypass with the countdown running, re-target + reset, revert to talking.
- **Surfacing:** the status bar shows the answering register (e.g. `✦ emotional`); a `/roles` command (registers, active hold, the models behind them); `/model` suspends routing for the session.
- Config: `LUMI_MODEL_ROLES` (off), `LUMI_ROLE_COOLDOWN` (~3), `LUMI_ROLE_CONFIDENCE` (~0.6), `LUMI_ROLES_FILE`.

**DoD:** with roles on, a hard task answers on `thinking`, a loaded message on `emotional`, small talk on `talking` — the lexical stage alone routes clear cases (no classifier call), the classifier only fires on *unsure* (and its cost is the tiny model); an escalated register **holds** for N turns, a "дякую" mid-hold is answered by talking while the hold keeps counting; the classifier prompt carries **no personal data**; classifier failure → talking; the status bar + `/roles` show the register; `/model` pins; **off → byte-identical**; the `{reply, emotion, intensity}` contract + per-user isolation untouched.

**Tests:** unit — the lexical table (closers, distress, code, length, override); the classifier parse + floor + failure→talk (mocked); the **firing matrix** (who fires when: clear-talk / clear-escalate / unsure→parallel); the hold (countdown, short-message bypass, re-target/reset, revert); profiles parse the four roles (+ defaults from `reply`). Contract — routing never alters the emotion contract; the classifier prompt contains no memory/personal fields (pinned). Integration — a mocked conversation walks talk → think → hold → bypass → revert; **byte-identical off**. **No paid calls.**
