...
- **`recall(scope=…)`:** extend the v0.31 tool with `scope = messages | facts | all`, routing the cosine search to the matching vectors; default `messages` → today's behavior unchanged; fact hits framed as **trusted recollection** (no de-id), deduped against the prompt. (The `thoughts` scope is added in v0.42.)
...
**Goal:** today **one model** (`claude-opus-4-8`) serves **every** LLM call — the visible reply, the thought stream, mood, summaries/facts/compaction, and every tool-loop step. Measured over a week of real usage (`.lumi/cache-log.jsonl`), the **reply is ~53%** of model spend, **tool steps ~21%**, **thoughts ~15%**, and **housekeeping ~11%** — yet bookkeeping and inner musings don't need a frontier model. This phase adds **per-operation model routing within the Claude tiers**: keep the visible **`reply` on Opus 4.8** (the voice — untouched), route **thoughts + mood to Sonnet 4.6**, and the **bookkeeping + mechanical tools to Haiku 4.5**. Off → one model, byte-identical. Target **~−26% of model spend with no change to her voice** — she *thinks and digs* on cheaper tiers but *speaks* on Opus. A runtime **`/mode`** dial flips the **reply tier** mid-session between **Opus 4.8** (quality, default), **Sonnet 4.6** (balanced), and **Haiku 4.5** (cheapest — *speak on Haiku* to save most), with the active mode on the status bar — a friendly three-state wrapper over the v0.37 toggle. Full measured analysis + design: [LLM_OPERATIONS_COST.md](../docs/LLM_OPERATIONS_COST.md) + [MODEL_ROUTING_IMPLEMENTATION.md](../docs/MODEL_ROUTING_IMPLEMENTATION.md).

The seam is **already there**: `model` is a per-call argument to `LLMClient.reply`/`reply_structured`, one Anthropic client serves all Claude tiers, and the **bounded tool-loop reuses the call's model** — so routing an *operation* routes its whole tool-loop for free (no new client, no new adapter; the cross-provider/engine-swap question is **deferred** — see the cost-doc appendices). Stays within Claude tiers; the `{reply, emotion, intensity}` contract + per-user isolation are untouched. Depends on: v0.1 (the `LLMClient` seam), v0.15 (prompt caching — the reply's Opus cache, the ~$232/mo saver, is preserved).

**Tasks:**
- **Layer 1 — per-operation routing (the foundation, no client change):** a `_model_for(kind)` resolver + per-tier config (`LUMI_MODEL_THINK` / `_MOOD` / `_HOUSEKEEPING`, each defaulting to `LUMI_MODEL`); **one line** in `_housekeeping_reply` (`model=self._model_for(kind)`) routes `think`/`mood`/`summary`/`facts`/`compaction`/`session-start` **and their tool-loops**; the `reply` stays on `LUMI_MODEL` (Opus). Unset → byte-identical.
- **Runtime reply-tier toggle (`/mode`):** a three-state mode command — **`/mode opus`** (quality, the default) / **`/mode sonnet`** (balanced) / **`/mode haiku`** (cheapest — *speak on Haiku* to save most); **`/mode`** alone prints the active tier — that re-points the **reply** model mid-session via the v0.37 `Core.switch_model` (Opus ⇄ Sonnet ⇄ Haiku, same Anthropic client), the status bar reflecting it. A friendly named dial over the per-op routing (the cheap ops are already routed); reuses v0.37 (`switch_model` + the `/model` mechanism), no restart, persists nothing (next start uses `LUMI_MODEL`). The tiers come from config aliases (`opus`/`sonnet`/`haiku`), not hard-coded.
- **`read_file` result-size cap (no model change):** cap `line_count`/chars on `read_file`/`read_around`/`search_files` — cuts ~29% of the `tool` bill with **no** coherence risk (a 220 KB book read bloats the next step regardless of model).
- **Layer 2 — per-step routing inside the reply tool-loop (optional, gated):** extend the client loop so *intermediate* read/write/navigation steps run cheaper while the **final visible step stays Opus** (the two-pass "dig on Sonnet, speak on Opus" pattern), behind `LUMI_TOOL_STEP_ROUTING`; mixing tiers in one message history carries a coherence risk → A/B before defaulting on.
...
**DoD:** with the tier vars set, `think`+`mood` run on Sonnet (and their tool-loops follow), housekeeping on Haiku, the visible `reply` stays on Opus + thinking; **`/mode opus` ⇄ `/mode sonnet` ⇄ `/mode haiku`** swaps the reply tier mid-session (no restart) and the status bar updates; the `read_file` size cap holds; cache-log shows the right model per `kind`; **off (unset) → one model, byte-identical**; the `{reply, emotion, intensity}` contract + per-user isolation are untouched; **no automatic `reply` downgrade** (the non-Claude engine swap is out of scope; `/mode sonnet` is an explicit, reversible user choice). Layer 2 ships behind its flag, off by default.

**Tests:** unit — a `MockLLMClient` records the `model` per call; a turn + a `think` + a session close use the configured tiers (`reply`→opus, `think`→sonnet, `summary`/`facts`→haiku); unset overrides → every call uses `self._model` (byte-identical guard); the `read_file` cap clamps the result; **`/mode opus`/`sonnet`/`haiku` re-points the reply model** (the mock records the swap) and an unknown mode is rejected. Contract — the routed `think`'s **tool continuations** are also Sonnet (the tool-loop follows the op); the emotion contract validated regardless of tier. Integration — measured per-kind cost shifts to the cheaper tiers; **byte-identical** off. **No paid calls** (model mocked).

### v0.41 — Thought scheduler (proactive thoughts on a clock — an in-TUI module)
...
Also a **tick service** in the TUI — a **fast** in-TUI timer for **lightweight, ephemeral** directives: fire-and-forget, **not** persisted, **a no-op if missed** (it runs only while the TUI is alive). This is the home of v1's **`%update_state`** (advance the inner-life/needs state to `now`): a missed tick costs nothing because the work is **advance-to-`now`** + idempotent, and the lazy session-start path catches any gap. (Durable directives like `%brief` instead use the scheduler's last-fired state + startup catch-up.)
...
- The **tick service** — a fast in-TUI timer for **ephemeral, non-persisted** directives (e.g. `%update_state`): fire-and-forget, collapse a backlog, drop a missed tick.
...
### v0.42 — Lean memory IV: thoughts (cap + thought recall)
...
### v0.43 — Semantic recall V: thematic recall (topic routing)
...
- A **`/discover-topics`** Claude Code skill (`.claude/skills/`): cluster this user's **stored vectors** → propose a draft `core/topics.md` (topic names + seed terms + representative exemplars) for human review; runs **offline over the store**, **never** mutates the live taxonomy or vectors. Works on the existing index — **independent of the rest of v0.43**, so it can run *before* the feature lands to author the initial set.
...
Three planning layers held in a **global** personal store (one Лілі — **not** per-user), advanced by a **pure, idempotent `update(state, now)`** (advance-to-`now`). It is driven **lazily at session start** *and* by the v0.41 in-TUI **`%update_state` tick** while the TUI is alive — the tick lives in **the brain (TUI), not a separate process**, and a missed one is a no-op (the lazy path catches any gap):
...
- A pure, idempotent **`update(state, now)`** (advance-to-`now`): **boundary detection** from the injected clock (new local day → a fresh today's plan; new ISO week → fresh intentions, unfinished carried over) via one housekeeping call per boundary. Driven **lazily at session start** + the v0.41 in-TUI **`%update_state`** tick (ephemeral, fire-and-forget) — so plans/needs stay fresh even without a turn.
