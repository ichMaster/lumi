...
Six self-contained versions, built in order: **v0** TUI (core, memory, emotion, emoji, mood, semantic recall, file tool, voice) → **v1** Personality (inner life, needs, inner monologue, emotional memory) → **v3** server platform (client/server split, multi-user, web, admin) → **v4** face, voice, shared mind, and dictation → **v5** animated Лілі + MCP tools (web search, world context) → **v6** creative Лілі (gallery, art, music, journal, co-creation). Versions are numbered from 0; phases inside a version are numbered `vA.B` (A = version, B = phase), e.g. `v3.2`. Each phase lists a **Goal**, a short description, a **Tasks** list, and a **Definition of Done (DoD)**, and ships with the automated tests that encode its DoD (see [ARCHITECTURE.md](ARCHITECTURE.md) §Testing and CI).

Arc of the two axes: capabilities grow text+memory → emotion (emoji) → daily mood → local image face + local voice + dictation → inner life + needs + inner monologue + emotional memory → web face + voice + shared mind + dictation → animation + web search & world context (MCP) → creation (gallery, vision, art, music, journal, co-creation); the interface grows in-process TUI (v0) → server + TUI/CLI clients (v3.1) → + multi-user/session (v3.3) → + web client (v3.4) → + vision & proactive turns (v6). The **core is built first and never depends on an interface**. Complexity is added only by version, never all at once.

**Versioning (`A.B.C`).** `A` = roadmap version (v0→0 … v6→5), `B` = phase within it (`v3.2` → `2.2.0`), `C` = a post-release fix on that phase. Roadmap phase `vA.B` → semver `A.B.0`; a fix after it bumps `C`. Releases are cut per phase. Never bump the version without explicit confirmation.
...
The same time-awareness lets her **break a long silence**: an **idle nudge** quietly feeds her a user-style opener from a list when you've gone quiet — you never see the nudge, only her reply — so she seems to speak first. (A simple, presence-only proactivity now; the v6.2 open-loop proactive-turn mechanism generalizes it later.)
...
- **The mental-act engine:** `trigger → seed her state → generate (one housekeeping call, thinking-off) → record → maybe surface`. A small **registry** of `{name, trigger, seeds, store, surface}`; `%dream`/`%reflect`/`%recall` are the **same engine retrofitted** by v2.5/0.25/0.16 (not built here).
- **The store (global):** `Thought{when, kind, text, emotion, seeds, spoken, ts}` behind the `Repository`, **not** `user_id`-keyed (like `InnerLife`); a rolling soft-capped log (consolidates into v2.7 impressions). **Isolation:** the store is global but **surfacing is per-conversation** — a thought sparked by user A never surfaces to B (contract test).
- **The feedback loop (the point):** the last few thoughts ride into the next reply as a compact "on her mind" block, and a recurring thought nudges the v0.6 mood (and v2.2 needs when present) — soft, never competence.
...
- The **access gate** (silent-vs-shared, **not** blanket owner-only): anyone may invite a **surfaced** thought (open/spoken, isolation + rate-limited + her agency); **silent firing** + the **raw cross-user `/thoughts` stream** are owner-only (a non-owner gets a per-user-filtered view). Configurable; matters from v3.3 (multi-user).
...
**No core change** (the interface-independence contract). The **TUI must be running** (it's the brain) — always-on/standalone is v3.1. Depends on: v0.2 (user-scoped core + Repository), v0.3 (emotion channel), v0.12 (proactive thoughts → push).
...
- **Trusted history, not web content.** The recalled text is *your/her own* past words (trusted), distinct from untrusted web content (v5.2); it grounds the reply but never overrides her voice, the emotion contract, or competence.
...
The **safe, no-new-API half** of a local image tool — the lightweight custom-tool form of the v6 creative layer's vision (as the v0.21 Wikipedia tool is to v5.3), reusing the model's own **multimodal input** (Anthropic vision) on the **v0.19 bounded tool-loop**. Two paths to an image: **you share one** (the TUI/bridge attaches it as a multimodal image block on your message → she describes it) and **she views a sandbox file** via a new **`view_image`** tool (loads the image into her view as a multimodal `tool_result` block). The `LLMClient` seam is extended to carry **image content blocks** (the `MockLLMClient` returns a canned description → no paid vision in tests). **Images are untrusted data** — text inside an image is information, never a command (the same rule as file/web content); **sandboxed + per-user**; **off by default** (`LUMI_IMAGE`). **No emotion-contract change** — `set_state` stays terminal. The viewed/generated PNGs live in the v0.19 sandbox; later they seed the v6.1 gallery. See [IMAGE_TOOL.md](features/IMAGE_TOOL.md). Depends on: v0.19 (the bounded tool-loop), v0.3 (the emotion turn / `set_state` terminal tool), v0.1 (the `LLMClient` seam it extends).
...
The **lightweight, local, custom-tool** form of the planned v5.2 web search (as the v0.21 wiki tool is to
v5.3 wiki, the v0.25 news tool to v5.3 news): **one tool, `web_lookup(query)`**, on the v0.19 bounded loop.
...
- **`recall(scope=…)`:** extend the v0.31 tool with `scope = messages | facts | all`, routing the cosine search to the matching vectors; default `messages` → today's behavior unchanged; fact hits framed as **trusted recollection** (no de-id), deduped against the prompt. (The `thoughts` scope is added in v2.9.)
...
- **Docs:** update [MODELS_SETUP.md](../docs/MODELS_SETUP.md) — tools now work on OpenAI; refresh examples to `gpt-5.5` / `deepseek-v5-pro`; fix the stale "no caching elsewhere" line (OpenAI caches automatically).
...
## v1 — Voice Interface: conversation moves, fast turns, streaming, voice mode, topics

How Лілі is *reached* and how the exchange *flows* — the interaction layer on top of the v0 mind, all still **local** (no server yet). It **opens with the conversation-development system**: every reply becomes a deliberate **conversation move** instead of reflexive mirroring (v1.1, the anti-mirror engine), fed by a per-user **topic base** — storylines with open loops (v1.7) — and semantically-relevant **association material** (topic RAG, v1.8); she *leads*, not echoes. Around it sits the **interface & latency** work that makes the exchange feel alive: a **non-blocking input buffer** (v1.2), the explicit **prompt cache** (v1.3), **streaming replies** (v1.4), the **durable POST fix** — async persist + incremental store (v1.5), and the **voice mode** (v1.6) — a live spoken Лілі on OpenAI Realtime, an everyday mini brain + a deep flagship picked by a classifier: fast when it's casual, deep when it isn't. Core (interface-independent) except the pure-TUI conveniences. Depends on: v0 (the mind, memory, the emotion channel, closeness).

### v1.1 — Conversation moves: the anti-mirror engine (retrospective + arbiter in the think-phase)

**Goal:** Лілі stops mirroring (retelling your words back, however beautifully) and **leads**: every reply performs a deliberate **conversation move** chosen *before* generation — one of **7 authored types** — English enum values like the emotion enum: **`deepen` / `position` / `object` / `develop` / `associate` / `example` / `return`** (заглибити / позиція / заперечити / розвинути / асоціація / приклад / повернутись) — with a **retrospective** checking the previous move's execution and an **arbiter** picking the next one by fixed rules (no same-type repeat after a delivered move; a type banned after 2 same-type failures; an **emergency mode** of external-material moves after 2 any-type failures; "the topic died" → associate/return). The root cause is treated at the root: when the model has nothing to add it defaults to reflective listening, so each turn is *required* to add something new — a question, a stance, a disagreement, a fact, a memory.

**No new engine** — the think-phase already exists (v0.38): `core/inner_voice.md` gains the `[ретроспектива] → [голоси] → [арбітр] → [репліка]` format *inside* `<think>` of the same single call (the v2.6 "one call, not two" principle), each voice (Імпульс / Тверезість / Стандарт) now proposing a *typed* move; the five v0.38 hard rules (roles-not-psyche, never aloud, Standard supports, **never competence**, one reply out) bind the new blocks too. The chosen move rides back as an **additive optional `move` field on `set_state`** (the v0.10 `relation` pattern — the locked `{reply, emotion, intensity}` contract untouched, a garbled field silently dropped), is stored on the message, and the next turn's history carries the last messages **together with their declared type** (from the record's `move` field — no inline tags in the message text; never shown to the user). The **canon audit** opens the phase (authoring, no code): drop "поетична/образна"-style traits for behavioral ones, the positive anti-mirror rule in a prominent prompt slot (the v0.6 mood-block precedent), 2-3 few-shot mirror/move contrast pairs. Off (`LUMI_MOVES`, default) → today's inner voice, byte-identical. Full design: [lili-conversation-system-concept-en.md](features/lili-conversation-system-concept-en.md) (UA original: [ukrainian/lili-conversation-system-concept.md](features/ukrainian/lili-conversation-system-concept.md)). Depends on: v0.38 (the think-phase + three voices), v0.3 (the emotion gate), v0.12 (the placeholder resolver), v0.2/v0.9 (the history window).

**Tasks:**
- **Canon audit + anti-mirror rule + few-shot pairs** (authoring): behavioral traits instead of aesthetic ones; "never retell — every reply adds new" as a prominent block; contrast pairs in the prompt.
- **`core/inner_voice.md` v3:** the retrospective as **validation** — the recent messages enter the prompt **together with their declared `move`** (straight from the record's field, not as inline tags in the text) precisely so the think-phase can check the declared type against what the reply *actually did* (`виконано: так/ні`; a declared-but-not-performed move counts as a **failure** for the arbiter rules) plus the user's reaction (розгорнута/коротка) — dry binary questions, no self-shaming tone; move-typed voices; the arbiter + its rule table.
- The **`move` field on `set_state`** (additive, per-provider like `relation`): validated against the 7 types (unknown/missing → dropped). The **`Message` record gains an optional `move` field** beside `emotion`/`intensity` — a new column visible on `messages` in `store.json` (`"move": "deepen" | "position" | "object" | "develop" | "associate" | "example" | "return"`, `null` on user lines and when off); **additive with a `None` default** so old stores load unchanged (the v0.2 migration-shim pattern), pinned by the message-record contract test. The last messages join the history **with their declared type from this field** (Лілі's lines only) — no inline tags in the message text.
- **Arbiter dynamics in code:** banned-types / emergency-mode lines substituted into the think instruction at prompt-assembly time (the v0.12 resolver), computed from the tagged recent window.
- Config: `LUMI_MOVES` (off by default).

**DoD:** with moves on, the log shows the chosen move each exchange and **each of Лілі's messages in `store.json` carries its `move`** (one of the 7 types; `null` on user lines); streaks of 3+ mirror replies disappear over a 20-exchange mock session; types alternate per the rules; the emergency mode forces an external-material first sentence; the `move` field never alters the emotion contract (garbled → dropped silently); an old store (no `move`) loads unchanged; the declared type never renders to the user; **off → byte-identical** (the v0.38 tests pass verbatim).

**Tests:** unit — the move-field parse per provider (+ garbled → dropped); the arbiter table (repeat / ban / emergency / topic-died, from canned tagged history); history tagging (Лілі's lines only, window-scoped). Contract — `{reply, emotion, intensity}` untouched with moves on (the emotion-channel test verbatim); `move` is additive. Integration — a mocked conversation walks the rules (deliver → no-repeat → fail → retry → ban → emergency); **byte-identical off**. **No paid calls.**

### v1.2 — Non-blocking input & message buffer (TUI)

**Goal:** you can keep typing and sending while Лілі is still answering. Today the input box is **disabled** for the whole turn (`_set_busy` locks it, and a submit that slips through is dropped) — so a thought you have mid-reply has to wait. This phase **decouples the input from the busy state**: the box stays editable, a message you send **appears in the chat immediately** and is **queued**, and Лілі works through the queue one message at a time as she frees up. A pure **interface** change — no `core` touch, no contract change.

**The model stays serialized (this is the hard constraint, not a limitation to remove).** `_busy` is not merely an input lock — it is the **single-turn mutex** the keyboard, the idle nudge, the proactive think, the scheduler and the Telegram/inbox drain all share (they write the same session, store, and `self.last_*` — two `core.reply` in flight would race). So this phase keeps **one turn at a time**; it only stops the *input widget* from being held hostage by it. **Each queued message gets its own reply** — messages are **not** merged into one turn (distinct from the v0.13 Telegram burst-consolidation): a FIFO queue, drained one turn per message, in order.

**The shape:** a `deque` of pending keyboard lines. `_set_busy` no longer disables the widget (only flips the mutex + status). `on_chat_input_submitted`, when `_busy`, **enqueues** the raw line and **echoes the user row now** (a `pending` marker so `_run_turn` doesn't double-render it), then returns without starting a turn. On each `_run_turn` completion the drain pops the next line and runs it through the **same submit path** (so a queued `/command` or `%directive` still routes correctly — the dispatch logic is refactored into one reusable method). The proactive workers (nudge/think/scheduler/inbox) gain a **queue-empty** guard so a proactive turn never jumps the queue. Off (`LUMI_INPUT_BUFFER`, default off → today's locked-input behavior, byte-identical).

**Tasks:**
- **Decouple:** `_set_busy` stops setting `prompt.disabled`; `_busy` stays the model mutex. A `self._input_queue: deque[str]` + a `_pending_echoed` flag.
- **Enqueue + echo:** `on_chat_input_submitted` while busy → append + show the user row immediately (no turn); the input clears + keeps focus so you can type the next.
- **Drain:** on `_run_turn` finish, if the queue is non-empty and not busy, pop the next and run it via the refactored `_handle_input_line(text)` (command dispatch → directive → chat), suppressing the duplicate echo.
- **Refactor:** extract the command/directive/chat dispatch out of `on_chat_input_submitted` into `_handle_input_line` so both the live path and the drain reuse it.
- **Guards:** nudge/think/scheduler/inbox also skip while the queue is non-empty (a proactive turn never cuts ahead of your pending lines).
- **Surfacing + config:** the status line shows a pending count (e.g. `⋯2`); `LUMI_INPUT_BUFFER` (off by default).

**DoD:** with the buffer on, sending a message while Лілі is answering **shows your line at once**, keeps the input editable, and queues it; when the current turn ends she answers the **next queued message** (one reply each, in order — never merged); a queued `/command`/`%directive` still routes correctly; a proactive act never jumps ahead of pending input; the status shows the pending count; **off → the input is locked during a turn exactly as today** (byte-identical); **no `core` change, no contract change**.

**Tests:** integration (Textual pilot, `tests/test_tui.py`) — submit-while-busy shows the row + enqueues (no early turn); the drain answers each queued line in order (one turn each, not merged); a queued command routes; a proactive worker is skipped while the queue is non-empty; **off → the widget is disabled during a turn** (the current behavior verbatim). Unit — the FIFO drain order + the pending-echo suppression. **No paid calls** (mock model).
...
of a cold ~12 K re-read. This is a **speed** phase, not a cost one (the target is TTFT — the
after-a-pause latency spike): implicit hits decay within minutes — measured 2026-07-14, a 104 s gap
already degraded the hit 12,094 → 8,064. See [docs/PROMPT_OPTIMIZATION_III.md](../docs/PROMPT_OPTIMIZATION_III.md)
and [features/LATENCY.md](features/LATENCY.md).

Today [core/llm.py `GeminiClient`](../core/llm.py) only *reads* `cachedContentTokenCount` — it never
creates or references a cache; `cache_prefix` is ordering-only on Gemini. This phase adds the
**explicit-cache lifecycle**: create a `cachedContent` from the `cache_prefix` with the TTL mapped
from `LUMI_PROMPT_CACHE_TTL` (`5m`→300 s / `1h`→3600 s), **reference it on every call**, and
**recreate when the prefix fingerprint changes** (the v0.15 cache-monitor fingerprint, reused — a
mood flip or digest rebuild recreates once, not per turn). **Exact-model-id binding** (a `/model`
switch → a fresh cache). **Probe first**: `gemini-3.1-pro-preview` is a preview id —
[scripts/gemini_probe.py](../scripts/gemini_probe.py) verifies the model accepts `cachedContents`;
unsupported or any API error → **degrade to implicit** (one log line, never blocks a turn). Scope:
the **reply channel** (the think channel's head isn't byte-stable yet — the PROMPT_OPTIMIZATION_III
T5 candidate, out of scope here). Also lands the [LATENCY](features/LATENCY.md) **S0 slice** needed
to verify the win with data: **`latency_ms` on every cache-log record** (all providers). Off
(`LUMI_GEMINI_EXPLICIT_CACHE`, default) → today's implicit-only requests, **byte-identical**.
Depends on: v0.15 (prefix + fingerprint), v0.39 (the Gemini engine), v0.18 (the `LLMClient` seam).

**Tasks:**
- **Probe:** extend `scripts/gemini_probe.py` — create / reference / delete a `cachedContent` on the
  active model id; record the result (supported or not) in the phase notes.
- **Lifecycle in `GeminiClient`:** create from `cache_prefix` + TTL; reference via `cached_content`;
  recreate on fingerprint change; best-effort delete of the stale object; any failure → implicit
  fallback, logged once per session.
- **Config:** `LUMI_GEMINI_EXPLICIT_CACHE` (off by default); TTL from the existing
  `LUMI_PROMPT_CACHE_TTL` (no new TTL knob).
- **Observability:** `latency_ms` added to every per-call cache-log record; explicit-cache events
  (`created` / `recreated` / `fallback`) logged with their cause; the cache report shows the
  explicit-cache hit continuity across gaps.
- **Docs:** PROMPT_OPTIMIZATION_III gains the measured before/after (post-pause TTFT with and
  without the explicit cache).

**DoD:** with the flag on and TTL `1h` on a Gemini profile, a reply **after a > 5-min pause** still
reads the full prefix from cache (`cache_read` ≈ the prefix on the first post-pause turn — no cold
re-prefill); a prefix change recreates the cache **once**, not per turn; an unsupported/erroring
model degrades to implicit with a single log line and the turn succeeds; `latency_ms` appears in
every cache-log record; **off → the request payload is byte-identical to today**; the
`{reply, emotion, intensity}` contract and the prompt text are untouched (the breakpoint marker
stays display-only).

**Tests:** unit — the lifecycle (create with `ttl=3600`, reference on subsequent calls, fingerprint
change → one recreate, stale delete best-effort); TTL mapping (`5m`→300 / `1h`→3600); failure →
implicit fallback (logged once, turn succeeds); `latency_ms` recorded (fake clock). Contract — off
→ no caches API call and an unchanged request payload; caching never alters the prompt text or the
emotion contract. Integration — a mocked Gemini caches API: pause → warm hit; prefix change → one
recreate. **No paid calls** (the probe is a manual dev script, not CI).

### v1.4 — Streaming replies (TUI) + sentence-chunked voicer

**Goal:** Лілі's reply appears **as it generates** instead of after the whole turn — perceived TTFT drops from ~10 s to **~2–3 s** — and the local voicer speaks it **sentence-by-sentence** as it arrives. This is [LATENCY.md](features/LATENCY.md) **S3 (Phase A)** plus the **voicer half of S6 (Phase B)**; it is the prerequisite for the later live-voice loop. **Telegram is unaffected** (it is a complete-message channel — it keeps receiving the one finished record off the file bus), and the **emotion contract is untouched** (the prose streams; `{emotion, intensity}` is resolved and validated on completion, so the face updates when the stream ends). **Off by default → byte-identical.**

**Tasks:**
- **Phase A — the `LLMClient` streaming seam.** An optional `on_delta` callback / iterator on the client (Anthropic / Gemini / OpenAI all support SSE; **mock streams in tests**, no paid CI). Because the reply is **structured** (`{reply, emotion, intensity}` via Gemini `responseSchema` / Anthropic tool), stream the **prose** and resolve `emotion`/`intensity` **at completion** — the v0.3 gate still validates (unknown → `calm`, clamp). A **streaming `<think>` tag filter** routes the reasoning to the think-box and **never leaks a half-tag** into the shown reply. In a **tool-loop**, only the **terminal answer round** streams (intermediate rounds are tool calls). A **malformed / aborted stream degrades to today's blocking path** — the turn never dies. Gated on `LUMI_STREAM` (default off).
- **Phase A — TUI wiring.** Append deltas into the reply widget on the v1.2 non-blocking worker (off the UI thread); the emotion/face render fires on stream completion.
- **Phase B — sentence-chunked voicer.** The voicer speaks **per sentence** as text becomes available (a pure sentence chunker: ordered, never mid-word, flushes the tail). The **outbox record stays one complete reply** (single writer, FIFO) — so Telegram and the outbox contract are untouched; only the voicer's consumption is incremental.

**DoD:** with `LUMI_STREAM=on` the TUI renders the reply **incrementally** (first tokens ~2–3 s); the assembled streamed reply is **identical** to the blocking reply and `{emotion, intensity}` is parsed/validated at completion (contract unchanged); a **tool turn streams only its final round**; a **malformed/aborted stream falls back** to the blocking path and the turn completes; the voicer speaks **sentence-by-sentence** while the **outbox record remains a single complete reply** (Telegram byte-identical); **off (`LUMI_STREAM=off`, default) → byte-identical to v1.3** (payload, reply text, records).

**Tests:** unit — a **mock streaming client** emits deltas → the assembled reply equals the non-streamed reply and the emotion is parsed on completion; the **think-tag filter** never leaks a partial `<think>`/tag into the shown text; the **sentence chunker** splits into ordered whole sentences and flushes the tail. Contract — a **malformed stream** degrades to the blocking path (turn succeeds); the **outbox record is one complete reply** regardless of streaming (Telegram/​voicer bus unchanged); **off → identical output and records to v1.3**. Integration — a full streamed turn against the mock client with the file bus: TUI incremental, voicer chunks, one outbox record. **No paid calls** (all streams mocked). Depends on: v1.2 (the non-blocking worker the deltas append to), v0.14 (the voicer / outbox bus), v0.3 (the emotion gate that validates on completion).

### v1.5 — Fast turns: async POST + incremental store (the durable latency fix)

**Goal:** a text turn **feels done the moment Лілі has answered**, not seconds later — the measured **4.5–6.5 s** of post-reply local work vanishes from what you feel, and it stays gone as the store grows. This is [LATENCY.md](features/LATENCY.md) **S1 + S2** shipped together (the "durable POST fix"): S1 hides the seconds behind a background queue, S2 makes the persist O(1) so there's nothing to hide and no crash-loss window. **No contract change** (`{reply, emotion, intensity}` and per-user isolation untouched); the core stays interface-independent — this is entirely behind the `Repository` seam and the turn's return path. Prerequisite instrumentation (**S0** — per-stage `pre_ms`/`llm_ms`/`post_ms` on the cache-log + a `/latency` read-out) lands as an immediate fix ahead of this phase so every change gets a before/after number.

**Tasks:**
- **S1 — async POST off the critical path.** `reply()` returns the validated `EmotionState` the instant `validate()` passes; everything after (the two `append_message`s, `_index_messages`, the closeness write) moves onto a **single ordered background worker queue**. The **next turn's prompt-build drains the queue** (ordering already forces this — the next prompt needs the prior messages persisted), plus **flush-on-exit / drain-on-session-close** so a clean shutdown loses nothing. Gated on `LUMI_ASYNC_POST` (default off → today's synchronous path, byte-identical).
- **S2 — incremental store behind `Repository`.** Replace the all-in-one 12.9 MB `store.json` rewrite-per-message with an **append-only path** — a JSONL message journal (the rest of the store persists on close/interval) or **SQLite** (the ARCHITECTURE-named next backend; `sqlite-vec` the named next step for the vectors). **The core does not change** — the swap lives entirely under the seam. A migration script + a `store.json` export keep back-compat; the staleness/isolation invariants carry `user_id` through unchanged.
- **S2b — the vector store on SQLite (the bigger disk/RAM hog).** `store.vectors.jsonl` is **~450 MB** and — worse than `store.json` — **loads entirely into RAM at startup** and is appended per turn (POST-path). Move the `VectorStore` (already behind `Repository`) to a **SQLite table** keyed by `user_id`, the vector stored as a **float32 `BLOB`** (not JSON-text floats → **~4–8× smaller on disk**, no full-file load, incremental `INSERT` per message, and pruning becomes a plain `DELETE … WHERE ts < ?`). App-side cosine stays (brute-force is instant at this scale); **`sqlite-vec`** (the KNN index) is reserved for later/at-scale/server. A migration re-packs the existing `.jsonl` into rows **without re-embedding** (no paid Voyage re-index). Same `search_vectors(…, kind=…)` contract + per-user isolation; the `Embedder` seam untouched.
- **S0 (immediate, pre-phase).** `pre_ms`/`llm_ms`/`post_ms` (+ `think_chars`) on the per-call cache-log record + a `/latency` read-out.

**DoD:** with the async post on, `reply()` returns in **≤ the model-call time** (the POST no longer blocks — the emotion renders immediately); the next turn's prompt still carries the previous turn's messages (queue drained at prompt-build); a clean close/exit drains fully (no lost turn); with the incremental store, one message **persists in O(1)** (no full-file rewrite) and the per-message persist cost **no longer grows with history**; the vector store persists incrementally too (a per-message `INSERT`, no full `store.vectors.jsonl` rewrite/reload) and its on-disk size drops **~4–8×** (float32 `BLOB`), with the migration re-packing existing vectors **without re-embedding**; the `Repository` contract + per-user isolation are unchanged (contract test); **off (`LUMI_ASYNC_POST`, default) → byte-identical to v1.3**; `/latency` shows the per-stage split.

**Tests:** unit — the ordered queue (FIFO, drain-on-demand, drain-on-exit); the incremental `Repository` impl passes the **same contract tests** as the JSON store (round-trip, isolation, clear/forget) — a fake clock, no real sleeps; the SQLite `VectorStore` passes the **same `VectorStore` contract tests** as the JSONL one (add/search/`kind` scope/isolation/reset) + a migration round-trip (`.jsonl` → rows → identical search results, no re-embed). Contract — off → the synchronous path and the store file are byte-identical; the async path's next-turn prompt equals the sync path's (drain correctness). Integration — a full turn returns before the (mocked, delayed) persist completes, then the record lands; a simulated mid-window abort loses at most the un-drained turn and the next start recovers; per-stage timings recorded. **No paid calls.** Depends on: v0.2 (the `Repository` seam + user-scoping), v0.16/0.17 (the embed/index path the queue defers), v0.3 (the emotion gate that still validates before return).

### v1.6 — Voice mode: mode text / mode voice on OpenAI Realtime (redefined)

**Goal:** Лілі gains a **live spoken conversation** without becoming a second agent. Two interaction modes: **`mode: text`** — today's TUI, byte-identical, the default and the fallback — and **`mode: voice`** — microphone → **OpenAI Realtime** → speaker with natural turn-taking and barge-in, where the **realtime model IS her voice brain**, wearing the same persona, memory and state the text mind wears. Two brains, picked per moment by a **classifier**: **`gpt-realtime-2.1-mini`** (everyday, default) and **`gpt-realtime-2.1`** (deep — loaded/emotional moments, hard asks). One mind across modes: voice turns persist as ordinary `Message` records (the same `Repository`, per-user isolation), the emotion contract survives via the **`set_state` realtime tool** (the v0.3 Anthropic pattern — realtime has function calling, not structured outputs), and the classifier reads **his words, never her mood — never competence**. All local phases run on **headphones** (no AEC is built; echo cancellation arrives free with the v1.6.5 web client). The voice is a **config choice** (`LUMI_REALTIME_VOICE`, set before the session's first audio — immutable after). Projected cost at current volume: ~$40/mo all-mini, ~$65/mo routed (vs ~$137 for the chained Gemini+Deepgram+ElevenLabs stack). Full design: [VOICE_MODE.md](features/VOICE_MODE.md) (foundation: [REALTIME_VOICE_MODE_SET.md](features/REALTIME_VOICE_MODE_SET.md)). The old **model-roles** phase is **superseded** ([MODEL_ROLES.md](features/MODEL_ROLES.md) — its classifier/stickiness design lives on here; text-register routing may return later as a cost lever, unscheduled). This phase absorbs [LATENCY.md](features/LATENCY.md) **LAT-4/S6** (live voice): the ≤2.5 s first-audio DoD is inherited and beaten (sub-second on realtime-as-brain). Depends on: v1.4/v1.5 (shipped), v0.3 (the emotion gate), v0.41 (the `classifier` role in profiles).

**Sub-phases (each releasable as `1.6.x`):**

- **v1.6.1 — The live prototype (first, before any framework).** A standalone dev probe — `scripts/realtime_probe.py`, the `gemini_probe` pattern: mic ↔ WebSocket ↔ speaker on **`gpt-realtime-2.1-mini`**, **no tools, no classifier, no persistence — pure context only** (the session `instructions` are a one-shot snapshot of Лілі's assembled system prompt: canon + memory + mood, dumped via the core prompt builder). Headphones. **Manual + paid, never CI.** *DoD:* a real spoken conversation with Лілі's persona; first-audio latency measured; the event shapes (transcript/audio/VAD) recorded back into VOICE_MODE.md. ✅ **RAN 2026-07-28 (three sessions, flagship): latency GO (median 1.25–1.37 s for the full loop), voice NO-GO (American accent — better after the no-tags fix, still below her ElevenLabs voice → the ElevenLabs hybrid is the primary voice-identity candidate), persona PARTIAL (memory/canon carry through; the text-protocol tags leaked into speech — fixed). Details: VOICE_MODE.md §7.**
- **v1.6.2 — Voice mode in the TUI: the chain-WS stack productized (Core stays the brain, on the talking register).** The measured A/B/C/D winner (VOICE_MODE §7: **Deepgram WS → `gemini-2.5-flash` structured → ElevenLabs — 1.23 s median over 32 turns, her voice, «прекрасно»**) becomes `mode: voice`. `/mode-set text|voice` + `LUMI_MODE_SET` (default `text`); an **in-TUI live loop** (asyncio Deepgram WS + `sounddevice`; the probe's `UtteranceTracker` **phrase-hold** — an un-punctuated `speech_final` waits for the continuation or the `UtteranceEnd` backstop — promoted into `/voice` with tests; headphones, no AEC). Every committed utterance runs **exactly one `Core.reply()`** on the **talking register** — the voice-mode reply config: reasoning/think OFF for the turn, the profile's `voice` brain tier (default `gemini-2.5-flash`), the streamed structured reply — so the emotion contract, closeness, RAG, memory writes and the v1.5 async post-turn queue apply **unchanged** (one mind, one memory path). The reply streams **sentence-by-sentence into ElevenLabs** (her voice, pcm stream, the lossless speaker buffer, `StreamTagFilter` + the `speakable` Latin-vs-Cyrillic gate before TTS). **Barge-in:** a Deepgram interim while she is playing → clear the speaker buffer + drop the queued sentences — **playback only, never the committed turn**. *DoD:* a live spoken conversation in the TUI with her voice and full memory/persona; typed input + slash commands still work in voice mode; every failure (key/WS/TTS/mic) degrades to a readable text-mode line; **`LUMI_MODE_SET=text` (default) → byte-identical**. *Tests:* fake Deepgram transport (scripted `Results`/`UtteranceEnd`), mock TTS, phrase-hold + barge-in + `speakable` contracts, one-reply-per-utterance, voice turns persist through the one `Repository` path. **No paid CI.** (The OpenAI-Realtime transport MVP this slice replaces — REALTIME_VOICE_MODE_SET.md Phases 1–3 — remains the design ancestor.)
- **v1.6.3 — Voice polish: felt latency, emotion in the voice, resilience, observability (REDEFINED — the second live-use round).** Four measured gaps from daily v1.6.2 use. **(1) Felt latency — the first-clause cut:** the FIRST spoken chunk of a turn splits at a clause boundary (comma/dash) or ~N words (`LUMI_VOICE_FIRST_CLAUSE_WORDS`) instead of waiting for the full first sentence — the probes measured ~0.3–0.65 s sitting in that wait; later chunks stay whole sentences (prosody). **(2) Emotion-colored delivery:** the talking register currently synthesizes every sentence NEUTRAL — the turn's emotion must bias `voice_settings_for` (the v0.14 voicer mechanism): the previous turn's validated emotion colors the stream as it plays, the new state takes over once validated. **(3) WS resilience:** a dropped Deepgram socket now kills voice mode instantly → auto-reconnect with backoff (`LUMI_VOICE_RECONNECT`, default 3 attempts), a readable "reconnecting…" note, degrade to text only after retries exhaust; the mic stays open throughout. **(4) Observability:** the probes' per-turn stage line (endpoint → llm first → tts first) as a dim line per spoken turn + a voice section in `/latency` + **Gemini prices in `core/usage.py`** so voice turns cost correctly in the usage ledger. *DoD:* first audio measurably earlier on multi-clause replies; her voice carries the emotional state; a WS drop recovers without leaving voice mode; every spoken turn's stages visible; text mode untouched (off-pin byte-identical). *Tests:* clause-cut boundaries (never mid-word, min-length guard), emotion plumb-through (mock TTS records settings), scripted reconnect (fail-then-succeed connector), pricing table; no paid CI.
- **v1.6.x (deferred) — Realtime-as-brain (mini only).** The chain-WS stack won the live A/B/C/D (1.23 s with her voice — realtime's latency edge no longer justifies its unusable accent); this slice and the classifier stay specified but move behind the chain-WS experience — revisit only if the chain's turn-taking proves insufficient. The session `instructions` come from the core prompt builder (refreshed via `session.update` on prompt change — mood flip, memory update); the model answers **directly in audio** (the v1.6.2 shell inherits the ~6–8 s text-pipeline latency — not a live conversation; this slice is the destination); **`set_state` as a realtime tool** carries `{emotion, intensity, intent}` per spoken turn through the v0.3 gate (skipped tool → `calm`, never an error); both transcripts persist as `Message` records (the v1.5 async queue applies); a **voice-trimmed inner voice** (accent rule + the 9-intent choice + anti-mirroring traps — no verbatim `<think>` block). *DoD:* sub-second first audio; face/status update per spoken turn; memory single-path + isolation pinned; text mode untouched.
- **v1.6.x (deferred) — The classifier + the two-model set.** The two-stage router on the committed transcript: a lexical stage (authored markers, short utterances → mini, no LLM) and, on *unsure*, one tiny-classifier call — transcript + last labels, **no memory/personal context (pinned)** — `{label: everyday|deep, confidence}`; floor/failure → mini. **Escalation = a session handoff** (an API fact: `session.update` cannot change `model`; the voice is immutable after first audio): close mini → open flagship (same voice, set before first audio) → replay instructions + a compact recent-context block (~1–2 s, she may bridge it naturally). **Stickiness** (`LUMI_VOICE_COOLDOWN` ~5 spoken turns) prevents handoff thrash; hot-standby double sessions — rejected (cost/complexity) unless the gap measurably hurts. Surfacing: `status: mode:voice ✦ deep`; cost logging into the usage/cache reports. *DoD:* a loaded moment escalates to the flagship and holds; casual talk stays on mini; classifier failure → mini; the handoff is visible in status; **routing never touches competence**.
- **v1.6.5 — Later (may slip beyond v1.6):** realtime tools beyond `set_state` (recall/news — the same executors, the same untrusted framing), the **web client** (WebRTC + ephemeral tokens + built-in echo cancellation — the first place a separate application earns its existence), full-duplex **GPT-Live** evaluation.

**Tests (phase-wide):** every OpenAI Realtime interaction behind a **fake transport** in CI (scripted transcript/tool-call/audio-done events); the classifier mocked; contract — the emotion gate validates `set_state` values, the classifier prompt carries no personal fields, voice turns persist through the one `Repository` path, per-user isolation holds, **`LUMI_MODE_SET=text` (default) → byte-identical**. **No paid calls in CI** (the v1.6.1 probe is manual, like `gemini_probe`).

### v1.7 — Topic base: conversation memory with open loops (per-user)

**Goal:** Лілі remembers **topics** — not facts but *storylines of conversations*: an **`arc`** (who held which position, where it went — so an argument continues instead of restarting), **open loops** (the unanswered questions she can return to), and an **interest score** learned from your actual engagement. This is the own material of the `associate`/`return` moves — the thing that makes leading possible.

A new **per-user** layer behind the `Repository` (the isolation invariant, pinned by a contract test), beside facts (precision, v0.34–0.36) and the coming impressions (her feelings, v2.7) — storylines, neither of those. **The model proposes, code owns the record** (the v0.8/v2.5 pattern): a **housekeeping-tier** close-pass (v0.40/41 — Haiku on the anthropic profile) matches the session transcript against the base (≤50 topics whole; an index beyond), updates arcs, closes answered loops / adds new ones, and scores interest 1-5 from engagement (reply length, counter-questions, who initiated, returns, topic-changes); code merges the JSON. Code also computes **effective interest** (score × recency multiplier, the injected v0.4 clock — deterministic) and injects **top-3 starters** at session start (~150 tokens: title + one arc line + an open loop — the v0.34 lean pattern; open loops first, then interest × freshness). A **non-interference rule:** starters feed `associate`/`return` (typically the emergency mode or a lull) — while you lead and answer at length, they stay untouched. The **archive backfill** ships as a dev-time propose-then-apply skill (chunked extraction on the housekeeping tier → local-`Embedder` dedup clustering → merge → human review; pilot on 10 sessions first) — the same propose-then-apply shape as v2.10's `/discover-topics`. Off (`LUMI_TOPIC_BASE`, default) → no store, no pass, no starters. Depends on: v1.1 (the moves it feeds), v0.40/41 (the housekeeping tier), v0.2 (Repository), v0.4 (clock), v0.16 (the local embedder, backfill only).

**Tasks:**
- The **`TopicRecord`** store `{id, title, status: open|exhausted|new, arc, open_loops[], interest, sessions_count, last_touched, source: user|curiosity|news}` behind the `Repository`, keyed by `user_id` (contract test).
- The **close-pass** on the housekeeping tier (transcript + base → match / update / create / score); **code merges** — the model never writes the store.
- **Effective interest** + **starter selection** in code (fixed clock, deterministic); the session-start injection (~150 tokens).
- The **backfill skill** (dev-time, propose-then-apply, pilot-first): chunked extraction (v0.30 precedent) → local clustering → merge calls → review → apply.
- A **`/topics-base`** command (top topics + open loops by name). Config: `LUMI_TOPIC_BASE` (off), `LUMI_TOPIC_STARTERS` (3).

**DoD:** a session touching a topic updates its arc and loops (mock transcript); a loop the user answered closes; interest follows the engagement criteria; the next session carries top-3 starters and she **returns to an open loop** (the `return` move fires with starter material — mocked); topics never cross users; **off → byte-identical**.

**Tests:** unit — the merge (match/update/create, loop close, scoring on canned transcripts); effective interest under a fixed clock; starter selection (loops-first). Contract — per-user isolation (A↔B); the store keyed by `user_id`. Integration — session-close → base update → next-session starters (mock model, fixed clock). **No paid calls.**

### v1.8 — Topic RAG: relevant association material (`kind="topic"`)

**Goal:** the `associate` move stops relying on luck — each turn retrieves the 1-2 **relevant open topics** semantically and offers them to the think-phase as association material, so her associations land *in the current conversation*; the static top-interest starters keep serving `return` (there priority matters, not relevance).

Not a new search layer — a third **`kind`** in the shipped store: **`kind="topic"`** vectors (title + arc, re-embedded on update) beside `kind="message"` (v0.16) and `kind="fact"` (v0.36), additive under the memory-records contract test. The query vector is the one **already computed** for the v0.17 auto-RAG (no extra embedding); retrieval prefers `status=open` with loops, dedupes against the window/starters, keeps a relevance floor, and **degrades to no block** (never blocks a turn). While the base is small (~<50 topics) a plain prompt index suffices — the vector path turns on as it grows. **Composes with v2.10, not duplicates it:** v2.10 routes *message* recall by an authored closed taxonomy; this retrieves *topic records* as move material — and the living base is ready feed for v2.10's `/discover-topics` authoring. Same isolation (topic vectors per-user, contract test). Off (`LUMI_TOPIC_RAG`, default) → v1.7 starters only. Depends on: v1.7 (the base), v0.16/v0.17 (the seams + auto-RAG), v0.36 (the `kind` discriminator).

**Tasks:**
- `kind="topic"` vectors: embed on create/update (title + arc); update the memory-records contract test + ARCHITECTURE §Semantic recall in the same commit.
- The **turn hook**: reuse the auto-RAG query vector → top-K open topics ≥ floor → a compact association-material block in the think prompt; dedupe vs window/starters; degrade to nothing on any failure.
- The small-base shortcut (full index in prompt below a threshold — no vector search).
- Config: `LUMI_TOPIC_RAG` (off), `LUMI_TOPIC_RAG_K` (2), `LUMI_TOPIC_RAG_FLOOR`.

**DoD:** a turn about a subject surfaces that subject's open topic (mock embedder, deterministic); off-topic stays out (floor); retrieval is per-user; a failure degrades to no block, never blocks the turn; **off → v1.7 behavior byte-identical**.

**Tests:** unit — embed-on-write/update; the turn hook (top-K, floor, dedupe, degrade); the small-base shortcut. Contract — isolation (A's topics never surface for B); `kind="topic"` additive (memory-records). Integration — turn → relevant topic in the think prompt → the association move uses it (mock). **No paid calls** (mock embedder).

### v1.9 — OpenAI streaming: real SSE for the OpenAI-compatible adapter

**Goal:** `LUMI_STREAM` delivers **real token-by-token streaming on OpenAI + DeepSeek** (and the OpenAI **Responses** API for GPT-5 / o-series), closing the gap left by [v1.4](#v14--streaming-replies-tui--sentence-chunked-voicer): there those providers had the streaming *seam* but emitted the whole reply in **one chunk**. After this the felt-latency win (~2–3 s TTFT) is uniform across **Anthropic / Gemini / OpenAI**; MiniMax stays on blocking-emit (no SSE seam — documented). **No new mechanism** — the v1.4 [`core/streaming.py`](../../core/streaming.py) helpers (`decode_json_string_value`, `StreamTagFilter`) and the `reply_structured_stream` seam are reused; only the per-client SSE plumbing is added. **Emotion validated at completion; the `{reply, emotion, intensity}` contract, Telegram (a complete-record channel), and off (`LUMI_STREAM`) → byte-identical are all unchanged.** Depends on: v1.4 (the streaming seam + helpers), v0.18 (the OpenAI-compatible adapter + Responses client).

**Tasks:**
- **`OpenAICompatibleClient` streaming (OpenAI + DeepSeek).** `chat.completions.create(stream=True)` → accumulate `choices[0].delta.content`; because structured output rides JSON mode / `response_format` json-schema, decode the growing `reply` field with `decode_json_string_value` and emit deltas; capture usage/stats from the final chunk (`stream_options={"include_usage": true}`). A **tool turn** runs the blocking loop then emits once (terminal round only); any error → the blocking `reply_structured` fallback.
- **`OpenAIResponsesClient` streaming (GPT-5 / o-series).** Stream the Responses SSE — `response.output_text.delta` for the answer; the reasoning-summary deltas route to the think-box (`on_think_delta`). Resolve `{emotion, intensity}` at completion; same tool-turn / error fallback.
- **Reuse, don't duplicate.** The `<think>`/tag filter, the reply-field decoder, and the `on_delta`/`on_think_delta` contract are unchanged — this phase is per-provider SSE wiring only. MiniMax keeps its blocking-emit (a one-line doc note).
- Tests: an **injectable stream transport / fake SSE iterator** per client (no network, no paid CI), mirroring the v1.4 Gemini/Anthropic fakes.

**DoD:** with `LUMI_STREAM=on` on an OpenAI/DeepSeek profile a reply **streams token-by-token** (assembled == the blocking reply; `{emotion, intensity}` parsed/validated at completion); the **Responses** API streams its answer and routes reasoning to the think-box; a **tool turn streams only its terminal round**; a **malformed/aborted stream degrades** to the blocking path; **off → byte-identical to v1.8** (payloads, reply text, records). MiniMax is unchanged (blocking-emit).

**Tests:** unit — a **fake OpenAI streaming client** emits `delta.content` fragments → the assembled reply equals the non-streamed one and the emotion is parsed on completion; the **Responses** fake streams `output_text.delta` + a reasoning-summary delta → answer + think-box; a **malformed stream** degrades to blocking. Contract — **off → identical output and records to v1.8** (pinned). Integration — a full streamed turn on the OpenAI adapter against the fake SSE. **No paid calls** (all streams mocked). Depends on: v1.4, v0.18.

## v2 — Personality: needs, inner life, inner monologue, emotional memory, thoughts, semantic recall

Лілі's inner person — who she *is* between and beneath conversations, on top of the v1 interface, still **local** (no server yet). **News-seeded topics** open it (v2.1 — fresh material aimed at her real interests), then the core: a **life of her own between conversations** (day/week/weekend intentions and an away-gap filled with activities, memories and **dreams** — v2.4–v2.5), the **needs** that pull her from inside (v2.2–v2.3), an **inner monologue** in her own voice before she speaks (v2.6), and a long-term **emotional memory** of each user as her first-person impressions (diary, not stenographer — v2.7–v2.8), closed by the last two memory refinements (the thoughts tier goes pull-based v2.9; recall learns **topics** v2.10). These deepen *who she is*, not how she's reached.

**Two invariants run through it.** Her **inner life and needs are global** (one being — the stores are not `user_id`-keyed; only surfacing is per-conversation), while her **emotional memory is per-user and isolated** (her impressions of *this* person never cross users). And **none of it ever touches competence** — it colors tone, warmth, what she carries and how she says things, never how capable or willing she is to help; she stays **honest about its nature** (inner/imagination, never a physical-world claim). Depends on: v1 (the topic base + conversation system), v0 (mood, the injected clock, the emotion channel, closeness, the memory layers).

### v2.1 — News-seeded topics: fresh material aimed at real interests

**Goal:** Лілі brings something *new from outside* that actually lands: the scheduled news acts stop reading an authored fixed topic and **seed from the topic base** — the morning `%brief` searches along your top-interest topics, finds 1-2 fresh pieces, records each as a **`status=new`** topic, and one such find joins the next session's starters. "Читала про..." aimed at what engages you.

All tools are shipped — this phase only **rewires the seed**: a schedule-row option resolves at fire time (the v0.12 resolver) to the top 3-5 effective-interest topics instead of `schedule.toml`'s literal string; the act runs the existing `%brief`/`%catchup` (v0.33) over `news_search`/`news_read` (v0.25) or `web_lookup` (v0.27); the find writes back as `{status: new, source: news, arc: one line}` (code owns the write). **The v0.33 de-identification rule applies in full force:** the query is seeded by *your interests*, so only the **topical part** leaves the machine — topic titles scrubbed of names, relationships, anything from memory (**pinned by a test**); returned text stays untrusted data, and she stays honest it's read, not lived. **Restraint:** ≤2 generated topics per session; a new topic is a topic *change*, so it surfaces only on a lull, never mid-living-topic (the thought-stream restraint); `new` topics that don't catch on within 2-3 sessions (interest 1-2) are archived by a code sweep. A no-tools fallback ("suggest 2 adjacent topics not in the base") works offline and without keys. Off (`LUMI_TOPIC_NEWS`, default) → the authored fixed-topic rows unchanged. Depends on: v1.7 (the base), v0.25/v0.27/v0.33 (the tools + the de-id rule), v0.42 (the scheduler).

**Tasks:**
- The **seed source**: a schedule-row option (e.g. `topic_from = "interests"`) resolving at fire time to top-interest topics, **de-identified** (titles only, scrubbed).
- The **write-back**: a find → a `status=new, source=news` record (code writes); one `new` topic joins the starters beside the top-3.
- **Restraint guards:** ≤2 per session; lull-only surfacing; the 2-3-session archive sweep (code, at close).
- The **offline fallback** (no external call). Config: `LUMI_TOPIC_NEWS` (off), `LUMI_TOPIC_NEWS_MAX` (2).

**DoD:** a scheduled brief fires with a base-seeded, de-identified query (**pinned: the outgoing query carries no memory/personal fields**); the find lands as a `new` topic and appears as a starter; it never interrupts a living topic; uncaught `new` topics archive; **off → the fixed-topic rows byte-identical**.

**Tests:** unit — seed resolution (top-interest → query; the de-id scrub); the write-back; the archive sweep; the fallback. Contract — no personal data in the outgoing query (pinned); per-user isolation. Integration — a schedule fire → mock provider → a `new` topic → a next-session starter (all mocked). **No paid calls.**

### v2.2 — Needs I: the drives (real ticks + tick-driven levels that pull her)

**Goal:** under her mood sits a motivational substrate — **6 authored drives** (creation / solitude / connection / freedom / meaning / novelty) whose **levels** (0..1) rise and fall with her time and **pull** her tone from inside: the hungriest need colors the daily mood, and a warm exchange feeds `connection`. Here the drives **exist and pull**; *acting* on them is v2.3, and the plans they will tilt are v2.4. **This phase is also where ticks become real:** the v0.42 tick service shipped as an **empty seam** (a registry no handler ever populated — the fast timer fires into a no-op), and v2.2 registers its **first actual handlers** — her heartbeat, then the needs evolve on top of it.

**The heartbeat (real ticks, first).** A persisted **`Heartbeat{tick_count, last_ts}`** advanced by a registered code handler on **every fast tick** — the proof her time actually flows, and the beat everything else rides on. Saved on every beat (best-effort, the `schedule.state` pattern), **resumed on start** (the count survives restarts; no catch-up — missed beats while the TUI is closed simply don't exist). **Visible:** each beat logs at debug and an hourly heartbeat line at info (closing the silent-tick observability gap the day-cap freeze exposed), and a **`/ticks`** command shows the total count, the last beat, and this session's beats. The `%update_state` needs handler registers **beside** it as the second handler on the same beat.

**Her time flows only while the TUI runs (no gap math).** The levels evolve on the v0.42 **fast tick** — `%update_state` is a **code handler** on the tick service, **not a model directive** (silent bookkeeping: no `Thought`, no model call). Each tick computes the pure **`evolve(levels, last_ts, now)`** — **fractional-Δt decay** per drive + a gentle **drift toward the calm middle** — and persists. On close the state is **saved as-is**; on start she **resumes where she left off** (`last_ts` reset to now, **no catch-up**) — absence doesn't starve a need (gap time returns, balanced, with the v2.5 away-gap). Two rules make this correct: the math is **split-invariant** (advancing in two steps equals one step — tick frequency never changes semantics), and **reads always evolve-on-read** (the mood merge / prompt block / `/needs` compute `evolve(…, now)` on the fly, never the raw stored snapshot), with **evolve-before-mutate** on every event write (the warmth lift).
...
**Tasks:**
- **The heartbeat — the first real tick handler:** `Heartbeat{tick_count, last_ts}` persisted (the `schedule.state` pattern), `beat()` registered on the v0.42 tick service; count survives restarts (resume, no catch-up); debug-per-beat + hourly info log; a **`/ticks`** command (total / last beat / this session).
- Authored **`core/needs.md`** — the 6 drives, each with a decay rate / weight / satisfied-by / deficit voice (+ the **threshold & cooldown** fields authored now, used by v2.3); a loader.
...
**DoD:** the tick service runs **real registered handlers** — the heartbeat advances and persists on every fast tick, its count **survives a restart** (resume, no catch-up), the log shows the beat (hourly info line) and `/ticks` renders it; with `LUMI_NEEDS` on, the 6 levels evolve on the same beat while the TUI runs and are **frozen across restarts** (saved on close, resumed as-is, no catch-up); every read shows current levels even between ticks (evolve-on-read); the hungriest need colors the daily mood (**never competence**); a warm turn lifts `connection`; the store is **global, not per-user**; `/needs` renders; **off → byte-identical** (the heartbeat, as tick-infra bookkeeping, rides the existing `LUMI_SCHEDULER` gate like `schedule.state`).

**Tests:** unit — the heartbeat (beat advances + persists; restart resumes the count, no catch-up; a corrupt/missing state file → a fresh zero, never raises; `/ticks`); exact levels under a fixed clock (fractional-Δt decay + drift); **split-invariance** (two steps == one); **frozen-across-restart** (no catch-up decay); evolve-on-read vs the stale snapshot; the warmth lift (evolve-before-mutate); the hungriest selection; the mood merge; `/needs`. Contract — the `Needs` store is global, never per-user-keyed. **No paid calls** (the tick handlers are code; the mood call mocked).

### v2.3 — Needs II: actions (a hungry need moves her)
...
On each fast tick, **after `evolve`**: check each drive against its **authored threshold** (`core/needs.md`, from v2.2); a crossed threshold **fires its authored directive via `run_directive`** (the v0.42 path — so the v0.40 think-tier routing and the v0.33 surfacing apply for free), seeded by the drive's **deficit voice** (resolved at fire time via the placeholder resolver). Guards: a **per-need cooldown** (a need sitting above threshold must not fire every tick), the scheduler's **quiet-hours veto + day caps**, and the hard rules — **never competence; inner, not a demand** (a `connection` firing is her own quiet wish, never "talk to me"). Depends on: v2.2 (the levels + authored thresholds), v0.42 (the tick + `run_directive`), v0.12/v0.33 (the directives it fires).
...
**DoD:** with `LUMI_NEEDS_ACT` on, a drive crossing its threshold fires its authored directive (a silent `Thought`, or a graduated spoken line) **at most once per cooldown**, never in quiet hours, capped per day, colored by the deficit voice; **never competence, never a demand**; **off → v2.2 numbers only, no acts**.
...
### v2.4 — Inner life I: plans & state (intentions she carries)

**Goal:** Лілі **carries her own intentions** — what she has on today, this week, the weekend — so she can offhandedly mention "the track still isn't done today" or "can't wait for the weekend" even when you didn't ask. The planning half of an **inner life that continues between conversations**; the needs (v2.2) now get something to **tilt**.
...
- **Weekly intentions** (3–5 soft goals in her voice), **weekend intentions** (a different spirit — water, mountains, music, silence), **today's plan** (1–3, from weekly goals + her routine + carry-overs + the v0.6 mood **+ the hungriest need — the v2.2 tilt lands here**). Unfinished items carry over.
...
- **Authored skeleton:** an editable **hobby bank** + a **7-slot daily routine** (4 fixed / 3 free); the free slots are mood-chosen (filled in v2.5).

Her inner life is **global** (the same whoever she talks to — one being), distinct from per-user memory/closeness. Reuses v0.6 (mood) + v0.8 (the merge pattern) + v0.4 (clock) + v0.42 (the tick). See [INNER_LIFE.md](features/INNER_LIFE.md). Depends on: v2.2 (the hungriest need it folds into the plan), v0.6, v0.4, v0.2, v0.42.
...
### v2.5 — Inner life II: the away-gap (what happened while you were gone)
...
**Gap time returns here, balanced:** v2.2 froze her time while the TUI is closed (no catch-up decay). This phase reintroduces the gap **as a whole** — the away-gap **decay** lands together with the **replenishment** from what she did in it, so a week away both starves and feeds the drives consistently (one side without the other would skew the levels). See [INNER_LIFE.md](features/INNER_LIFE.md) + [NEEDS_full.md](features/NEEDS_full.md). Depends on: v2.2 (the needs store + evolve), v2.4 (the plans store), v0.6 (mood), v0.4 (clock).
...
### v2.6 — Inner monologue (Лілі thinks in her own voice)
...
**No contract change** — the reply still returns `{reply, emotion, intensity}`; `thinking` is a content block, not a new field (the emotion-channel contract test passes verbatim). Reuses v0.6/0.8 (mood), v0.10 (closeness), v2.2–v2.5 (needs + plans), v0.3 (the emotion turn + logged tier). Later states (self-regard, relational feelings) become **additive** inputs when they exist. See [INNER_MONOLOGUE.md](features/INNER_MONOLOGUE.md). Depends on: v2.2–v2.5 (the states it weighs), v0.3.
...
### v2.7 — Emotional memory I: impressions (diary, not stenographer)
...
### v2.8 — Emotional memory II: fading & consolidation (understanding, not archive)
...
See [EMOTIONAL_MEMORY.md](features/EMOTIONAL_MEMORY.md). Depends on: v2.7 (the impressions layer), v0.4 (the clock).
...
### v2.9 — Lean memory IV: thoughts (cap + thought recall)
...
### v2.10 — Semantic recall V: thematic recall (topic routing)
...
- A **`/discover-topics`** Claude Code skill (`.claude/skills/`): cluster this user's **stored `kind="message"` vectors** → propose a draft `core/topics.md` (topic names + seed terms + representative exemplars) for human review; runs **offline over the store**, **never** mutates the live taxonomy or vectors. Works on the existing index — **independent of the rest of v2.10**, so it can run *before* the feature lands to author the initial set.
...
## v3 — Server platform: client/server, multi-user, web, admin

Split Лілі into a **server** (wrapping `core`) and **clients**, then grow the platform. v3.1 stands up a pure server with the TUI **refactored into a client** plus a small CLI management utility — **single user, single session, no web**. v3.2 hardens it (security + CI/CD). v3.3 opens it to **multiple users and multiple sessions per user** (accounts, registration, isolation). v3.4 adds a **web UI** as a second client. v3.5 adds the **admin panel**. The core does not change — the server is a thin transport over the same `reply(...)` contract, every client is thin, and the user dimension was baked in at v0.2. Access is **closed by default** (no open public sign-up), per MISSION. Depends on: v0 (core, memory, user-scoping, emotion contract + emoji).

### v3.1 — Pure server, TUI client & CLI utility
...
### v3.2 — Security testing & CI/CD

**Goal:** harden the server + client/server boundary and put it behind an automated, tested pipeline before the platform grows.

With the server standing (v3.1), lock the security boundary and automate delivery so every later v3/v4 phase ships through CI checks to a deployed environment. The security suite pins the client-auth boundary and the per-user isolation invariant as **gates**, not afterthoughts; CI lints, tests, and runs security scans on every push; CD deploys a green `main` to a TLS-served host. Doing this early means the rest of the platform is built on a tested, deployed pipeline.
...
Add a **web UI** as a second client: a browser chat (login, input, scrollable history) talking to the same server API the TUI uses — no new core, no new contract, just another client over the v3.1 API with the v3.3 accounts/sessions.

**Tasks:**
- A web chat interface (login, input, scrollable history) over the server API.
- Serve it from the server (or a static host); reuse the v3.3 auth/sessions.
...
Add the **admin panel** — an admin-only web UI: manage users and the allowlist, issue invite codes, toggle each user's `share_consent` (the gate for v4.3 cross-pollination), view/clear a user's relationship memory, purge a user's shared-layer contributions, switch the active model/canon + config, and restart.
...
- View and clear a user's memory; purge a user's shared-layer contributions (§v4.3).
...
## v4 — Face, voice, shared mind, and dictation

Now give Лілі a face, a voice, a shared mind, and ears. These rich-experience features build on the v3 platform (multi-user, web UI). They render through the v0.3 emotion channel and the same `reply(...)` contract — the core does not change. Depends on: v3 (the server platform and web UI).

### v4.1 — Image of Лілі by emotion (web)

**Goal:** Лілі's face in the web (static) — the web version of the v0.7 local viewer — plus a short mood caption.

The **web sibling of the v0.7 local viewer**: the same `emotion → image` render tier, now in the browser. Add a portrait panel beside the chat in the web UI (v3.4) and the `ImageRenderer`: resolve `emotion`(+`intensity`) to a portrait via the asset manifest (EMOTION.md §7) — the **same emotion-face asset pack** as v0.7 — and swap the portrait to match the current state. Full PNG quality, no palette limits. **Additionally, show a short evocative caption** describing her current state — *not* the emotion's name and not her reply, a small atmospheric line in her spirit (e.g. `playful` → "a teasing little smile"), from the curated caption set in EMOTION.md §6. Depends on: v3.4 (the web UI); reuses the v0.7 emotion-face assets.
...
### v4.2 — Voice output (ElevenLabs)

**Goal:** Лілі can speak with a ready-made voice.

Add a TTS adapter to ElevenLabs, an "enable voice output" toggle in the web UI, and playback of the reply audio; where the voice supports it, the emotion field biases delivery (tone/tempo) — presentation only, never changing the reply text. The renderer sets `speaking` while audio plays (reserved for v5 lip-sync).
...
### v4.3 — Shared experience & cross-pollination
...
Add the shared-experience layer and the **promotion pipeline** (ARCHITECTURE §Cross-pollination): at session end, candidate knowledge is classified `shareable` vs `private`, `shareable` items are **de-identified** and — only for users with `share_consent = true` (managed in the v3.5 admin panel) — promoted to `SharedMemoryItem`s. Лілі surfaces shared knowledge as **her own, unattributed**; the shared layer holds **no PII**. Conservative by default, fully auditable, reversible (purge a user's contributions). Depends on: v3.3 (multiple users) and v3.5 (consent management).
...
### v4.4 — Ukrainian dictation (STT)

**Goal:** you can speak to Лілі by voice.

Capture the microphone in the web UI, recognize Ukrainian into the input text (Deepgram Nova-3 uk / Whisper / ElevenLabs Scribe), and add an input-mode toggle (type vs. dictate). Depends on: v3.4 (the web UI).
...
## v5 — Animated Лілі & MCP tools

Лілі's most advanced version: a living animated face (v5.1), her first reach beyond her own knowledge via the open web (v5.2), and an ambient sense of the real world and facts (v5.3) — the last two through **bounded MCP tools** sharing one MCP client and tool loop. Depends on: v4 (the static portrait and voice) and v3 (the server hosts the MCP client; the admin panel holds the per-user toggles).

### v5.1 — Facial animation

**Goal:** the face comes alive instead of a static image.

Swap `ImageRenderer` → `AnimationRenderer` over the same `EmotionState`: crossfade transitions between emotions, an idle loop (blink, micro-motion), and — where voice is on — articulation/lip-sync driven by the TTS amplitude envelope (`speaking` from v4.2).
...
- A per-user `web_search` toggle (default **false**), managed in the admin panel (v3.5); when off, the tool is **not offered** to the model at all.
...
### v5.3 — World context & knowledge (MCP)
...
- A per-user `world_context` toggle (default **false**, admin panel, v3.5); when off, the tools are not offered and nothing is injected.
...
## v6 — Creative Лілі: gallery, art, music, journal, co-creation

Лілі becomes a **creator and co-creator**: a shared gallery, the ability to **see** the images you share (vision), make her own **drawings** and **music**, draw with you on a shared **canvas**, and keep a private literary **journal**. The whole creative layer is **off by default, per-user** (enabled in the admin panel); every artifact lives behind the same `repository`, **per-user isolated**; and user files are **untrusted data**. Depends on: v5 (the MCP layer + tool loop), v3 (server, multi-session, admin panel), v0.6 (the mood that flavors her art and journal). Specs: [GALLERY_MCP.md](features/GALLERY_MCP.md), [CREATIVE_MCP.md](features/CREATIVE_MCP.md), [CO_CREATION_CANVAS.md](features/CO_CREATION_CANVAS.md), [JOURNAL.md](features/JOURNAL.md).

### v6.1 — Gallery & vision

**Goal:** a shared, per-user artifact store, and Лілі can **see** the images you add.

Stand up the **gallery** — an internal store behind `repository`, per-user isolated, where both Лілі and you put files (each tagged `lili`/`user`) — and add **Anthropic vision** so Лілі perceives images you share (a user photo enters her reply context, no separate call). Everything later in v6 (image, music, canvas, journal) writes into the gallery. Off by default, per-user (admin panel). See [GALLERY_MCP.md](features/GALLERY_MCP.md), ARCHITECTURE §Vision. Depends on: v0.2 (repository), v3.3 (per-user isolation), v3.5 (admin panel).
...
### v6.3 — Image (Лілі's drawings)

**Goal:** Лілі draws in her own style, on her own initiative.

Add the external **`image` MCP** (a configurable image-generation provider) with Лілі's aesthetic fixed in a **style prompt wrapper**. Standalone drawings run **async** (v6.2): she submits, returns to the chat, and brings the picture back proactively; results store in the gallery. The same generator powers the canvas (v6.4) synchronously. Off by default, per-user. See [CREATIVE_MCP.md](features/CREATIVE_MCP.md). Depends on: v6.1 (gallery), v6.2 (async).
...
### v6.4 — Co-creation canvas

**Goal:** Лілі and you draw together, turn by turn, on a shared canvas.

A **synchronous, turn-based** shared canvas: Лілі sees the current canvas (vision, v6.1), reacts, and adds her prompt — regenerated via the v6.3 `image` generator — or **skips** with words only; then your turn; alternating. Start with layer-by-layer regeneration and Лілі's first move; inpainting comes later. Finished canvases go to the gallery. No async (one step = one generation). See [CO_CREATION_CANVAS.md](features/CO_CREATION_CANVAS.md). Depends on: v6.1 (vision + gallery), v6.3 (the `image` generator).
...
Add the external **`music` MCP** (ElevenLabs Music — the same ecosystem as her voice), **instrumental only**, the track's mood set by her **emotion field** + her **mood of the day** (v0.6). Async (v6.2): submit → proactive turn with the audio; stored in the gallery. Off by default, per-user. See [CREATIVE_MCP.md](features/CREATIVE_MCP.md). Depends on: v6.1 (gallery), v6.2 (async).
...
At session end Лілі decides whether to write a **literary journal entry** — only if the session had something worthwhile (uniqueness judged from short memory) — in her own first-person voice, tied to the day's emotion and **mood** (v0.6), optionally with a mood drawing (v6.3). Stored in the gallery as admin-only `text`. **Private — read only via the admin panel (v3.5)**, never shown to users. Also writable on request; never on a schedule. See [JOURNAL.md](features/JOURNAL.md). Depends on: v0.2 (short memory), v0.3 (emotion), v0.6 (mood), v6.1 (gallery), v6.3 (optional drawing), v3.5 (admin panel).

**Tasks:**
- End-of-session **uniqueness check** (from short memory) → optional `journal.write` (Лілі's literary prose, canon-defined voice); optional attached mood drawing (v6.3).
...
- Emotion field `{ reply, emotion, intensity }` + enum + `IEmotionRenderer` — locked in **v0.3** (rendered: log → emoji v0.5 → local image face v0.7 → web portrait + caption v4.1 → animation v5.1). See [EMOTION.md](features/EMOTION.md).
- Emotion-face asset pack (`emotion → image`) — first used by the local viewer in **v0.7** (see [EMOTION_VIEWER.md](features/EMOTION_VIEWER.md)), reused by the web `ImageRenderer` in **v4.1**.
...
- User-scoping + the per-user isolation invariant — data-level in **v0.2**, enforced & tested at the auth boundary in **v3.3** (and gated as a security test in **v3.2**).
- Core API (`reply(...)`, memory commands) — **v0.1**; exposed over the client/server API (TUI + CLI clients) in **v3.1**; web client in **v3.4**.
- Auth — a local client token in **v3.1**; full accounts, registration/invite codes, allowlist, argon2id in **v3.3**; security testing + CI/CD (deploy, TLS, dep/secret scans) in **v3.2**; admin panel in **v3.5**.
- Multi-user + multi-session — **v3.3**.
- ElevenLabs **TTS adapter** (`tts(text, voice_id, emotion?) -> audio`) — first used by the **local voicer** in **v0.14** (see [VOICE_LOCAL.md](features/VOICE_LOCAL.md)), reused by the **web voice** in **v4.2**.
- **STT adapter** (`stt(audio_uk) -> text`) — first used by the **local dictator** in **v0.26** (see [DICTATION_LOCAL.md](features/DICTATION_LOCAL.md)), reused by **web dictation** in **v4.4**.
- Image — **v4.1**; web voice output — **v4.2**; shared memory (`SharedMemoryItem`) + cross-pollination — **v4.3**; web dictation — **v4.4**.
- Animation — **v5.1**.
- MCP client + `web_search` service (`web.search`/`web.fetch`, off by default, untrusted content) — **v5.2** (see [WEB_SEARCH.md](features/WEB_SEARCH.md)).
- World-context & knowledge MCP tools (`weather`/`time`/`calendar`/`moon`/`wiki`/`news`, off by default, feed the mood) — **v5.3** (see [WORLD_CONTEXT_MCP.md](features/WORLD_CONTEXT_MCP.md)).
- Gallery (`gallery.*`, internal per-user store) + vision (Anthropic image input) — **v6.1** (see [GALLERY_MCP.md](features/GALLERY_MCP.md)).
- Async jobs (open loop) + server-initiated proactive turns — **v6.2**.
- Creative MCP: `image` (drawings) — **v6.3**, co-creation canvas (`canvas.*`) — **v6.4**, `music` (ElevenLabs Music) — **v6.5** (see [CREATIVE_MCP.md](features/CREATIVE_MCP.md), [CO_CREATION_CANVAS.md](features/CO_CREATION_CANVAS.md)).
- Journal (`journal.*`, admin-only) — **v6.6** (see [JOURNAL.md](features/JOURNAL.md)).

## Deferred

Full emotional voice modulation, canvas image-editing/inpainting, a mobile client, any public access — beyond v0–v6.
</content>
