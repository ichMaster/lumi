...
- The **`move` field on `set_state`** (additive, per-provider like `relation`): validated against the 7 types (unknown/missing → dropped); persisted on the message via the `Repository`; injected into the history window as `[хід: …]` on Лілі's lines only.
...
**DoD:** with moves on, the log shows the chosen move each exchange; streaks of 3+ mirror replies disappear over a 20-exchange mock session; types alternate per the rules; the emergency mode forces an external-material first sentence; the `move` field never alters the emotion contract (garbled → dropped silently); tags never render to the user; **off → byte-identical** (the v0.38 tests pass verbatim).
