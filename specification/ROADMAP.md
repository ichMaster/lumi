...
- **`recall(scope=…)`:** extend the v0.31 tool with `scope = messages | facts | all`, routing the cosine search to the matching vectors; default `messages` → today's behavior unchanged; fact hits framed as **trusted recollection** (no de-id), deduped against the prompt. (The `thoughts` scope is added in v1.8.)
...
### v0.42 — Model roles: register-routed replies (talking / thinking / emotional)
...
### v0.43 — Thought scheduler (proactive thoughts on a clock — an in-TUI module)
...
- The **`%update_state` tick handler** (code, not model): evolve → persist, registered on the v0.43 fast tick; state saved on close, resumed on start (no catch-up).
...
- **Guards:** per-need cooldown state (persisted beside the levels), the quiet-hours veto + the v0.43 per-day caps; the deficit voice as the seed (placeholder resolved at fire time).
...
Three planning layers held in a **global** personal store (one Лілі — **not** per-user), advanced by the same **pure, idempotent `update(state, now)`** pattern as the needs — driven by the v0.43 tick (the `%update_state` code handler grows **boundary detection**; the first tick after a start covers a restart across a boundary):
...
Her inner life is **global** (the same whoever she talks to — one being), distinct from per-user memory/closeness. Reuses v0.6 (mood) + v0.8 (the merge pattern) + v0.4 (clock) + v0.43 (the tick). See [INNER_LIFE.md](features/INNER_LIFE.md). Depends on: v1.1 (the hungriest need it folds into the plan), v0.6, v0.4, v0.2, v0.43.
...
### v1.8 — Lean memory IV: thoughts (cap + thought recall)
...
### v1.9 — Semantic recall V: thematic recall (topic routing)
...
- A **`/discover-topics`** Claude Code skill (`.claude/skills/`): cluster this user's **stored `kind="message"` vectors** → propose a draft `core/topics.md` (topic names + seed terms + representative exemplars) for human review; runs **offline over the store**, **never** mutates the live taxonomy or vectors. Works on the existing index — **independent of the rest of v1.9**, so it can run *before* the feature lands to author the initial set.
