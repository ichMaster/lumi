...
How Лілі is *reached* and how the exchange *flows* — the interaction layer on top of the v0 mind, all still **local** (no server yet). It **opens with the conversation-development system**: every reply becomes a deliberate **conversation move** instead of reflexive mirroring (v1.1, the anti-mirror engine), fed by a per-user **topic base** — storylines with open loops (v1.7) — and semantically-relevant **association material** (topic RAG, v1.8); she *leads*, not echoes. Around it sits the **interface & latency** work that makes the exchange feel alive: a **non-blocking input buffer** (v1.2), the explicit **prompt cache** (v1.3), the **durable POST fix** — async persist + incremental store (v1.4), **streaming replies** (v1.5), and **register-routed model roles** (v1.6) — fast when it's casual, deep when it isn't. Core (interface-independent) except the pure-TUI conveniences. Depends on: v0 (the mind, memory, the emotion channel, closeness).
...
### v1.4 — Fast turns: async POST + incremental store (the durable latency fix)
...
### v1.5 — Streaming replies (TUI) + sentence-chunked voicer
