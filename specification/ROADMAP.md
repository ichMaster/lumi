...
thought-directives — lands with the thought-tools phase (v0.32). See [WEB_LOOKUP.md](features/WEB_LOOKUP.md).
...
### v0.32 — Thought-tools: the autonomous mind *acts* (`%lookup` / `%imagine` / `%catchup` / `%search` / `%recall` / `%prompt` …)
...
### v0.33 — Thought scheduler (proactive thoughts on a clock — an in-TUI module)
...
### v0.34 — Semantic recall V: thematic recall (topic routing)
...
- A **`/discover-topics`** Claude Code skill (`.claude/skills/`): cluster this user's **stored vectors** → propose a draft `core/topics.md` (topic names + seed terms + representative exemplars) for human review; runs **offline over the store**, **never** mutates the live taxonomy or vectors. Works on the existing index — **independent of the rest of v0.34**, so it can run *before* the feature lands to author the initial set.
