...
- **The file bus (FIFO + id pointers).** `inbox.jsonl` + `outbox.jsonl` — append-only JSONL (`{id, text, ts}`), **one writer + one reader each** (no locks); the consumer tracks the **last id** it processed (a tiny pointer file), id-based so trimming later is safe. (Shared infra the v0.14 voicer / v0.21 dictator later ride.)
...
### v0.22 — Wikipedia tool (custom tool: search & read)
...
- **STT adapter** (`stt(audio_uk) -> text`) — first used by the **local dictator** in **v0.21** (see [DICTATION_LOCAL.md](features/DICTATION_LOCAL.md)), reused by **web dictation** in **v3.4**.
