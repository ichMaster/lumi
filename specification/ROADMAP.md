...
- The **`%update_state` tick handler** (code, not model): evolve → persist, registered on the v0.41 fast tick; state saved on close, resumed on start (no catch-up).
...
- **Guards:** per-need cooldown state (persisted beside the levels), the quiet-hours veto + the v0.41 per-day caps; the deficit voice as the seed (placeholder resolved at fire time).
...
Three planning layers held in a **global** personal store (one Лілі — **not** per-user), advanced by the same **pure, idempotent `update(state, now)`** pattern as the needs — driven by the v0.41 tick (the `%update_state` code handler grows **boundary detection**; the first tick after a start covers a restart across a boundary):
...
Her inner life is **global** (the same whoever she talks to — one being), distinct from per-user memory/closeness. Reuses v0.6 (mood) + v0.8 (the merge pattern) + v0.4 (clock) + v0.41 (the tick). See [INNER_LIFE.md](features/INNER_LIFE.md). Depends on: v1.1 (the hungriest need it folds into the plan), v0.6, v0.4, v0.2, v0.41.
