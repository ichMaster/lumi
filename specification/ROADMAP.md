...
- **Split `build_system_prompt`** into a **stable prefix** (canon + answer/emotion/style instructions + memory: weeks/days/sessions/**facts-digest** + mood) and a **per-turn tail** (the ambient «# Зараз» now/here + the **closeness block — recomputed each turn** + the last-24h thoughts), so a per-turn block never sits *inside* the cached span. The blocks are order-independent — only their position changes.
...
**Tasks:**
- Reorder the prompt into a **stable prefix** vs a **per-turn tail** (ambient + closeness + thoughts go last, before `[MESSAGES]`).
