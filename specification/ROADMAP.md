...
**Goal:** Лілі stops mirroring (retelling your words back, however beautifully) and **leads**: every reply performs a deliberate **conversation move** chosen *before* generation — one of **7 authored types** (заглибити / позиція / заперечити / розвинути / асоціація / приклад / повернутись) — with a **retrospective** checking the previous move's execution and an **arbiter** picking the next one by fixed rules (no same-type repeat after a delivered move; a type banned after 2 same-type failures; an **emergency mode** of external-material moves after 2 any-type failures; "the topic died" → associate/return). The root cause is treated at the root: when the model has nothing to add it defaults to reflective listening, so each turn is *required* to add something new — a question, a stance, a disagreement, a fact, a memory.
...
- The **`move` field on `set_state`** (additive, per-provider like `relation`): validated against the 7 types (unknown/missing → dropped). The **`Message` record gains an optional `move` field** beside `emotion`/`intensity` — a new column visible on `messages` in `store.json` (`"move": "заглибити" | "позиція" | "заперечити" | "розвинути" | "асоціація" | "приклад" | "повернутись"`, `null` on user lines and when off); **additive with a `None` default** so old stores load unchanged (the v0.2 migration-shim pattern), pinned by the message-record contract test. The last messages join the history **with their declared type from this field** (Лілі's lines only) — no inline tags in the message text.
...
**DoD:** a session touching a topic updates its arc and loops (mock transcript); a loop the user answered closes; interest follows the engagement criteria; the next session carries top-3 starters and she **returns to an open loop** (the `повернутись` move fires with starter material — mocked); topics never cross users; **off → byte-identical**.
...
**Goal:** the `асоціація` move stops relying on luck — each turn retrieves the 1-2 **relevant open topics** semantically and offers them to the think-phase as association material, so her associations land *in the current conversation*; the static top-interest starters keep serving `повернутись` (there priority matters, not relevance).
