...
- **v1.6.2 — Mode framework + transport MVP (Core stays the brain).** `/mode-set text|voice`; the `RealtimeVoiceSession` adapter (WS in the TUI process, `semantic_vad`, `interrupt_response=true`, `create_response=false` — Realtime never improvises before Core replies); committed transcripts run **exactly one `Core.reply()` each** and the returned text is spoken verbatim; **barge-in cancels audio playback only, never a committed turn**; `/model-set` mid-voice changes the text brain from the next turn without restarting audio, changing the realtime model/voice **requires a session restart** (pinned). Voice config: `LUMI_MODE_SET`, `LUMI_REALTIME_MODEL`, `LUMI_REALTIME_VOICE`, `[modes.voice]` in models.toml. *DoD:* live voice loop in the TUI; typed input + slash commands still work; every failure (key/connection/empty transcript/renderer) degrades to a readable text-mode line; **off → byte-identical**. *Tests:* fake realtime transport (scripted events), mode config parse/fallback, one-reply-per-transcript, restart-on-model-change, contract — voice never bypasses `{reply, emotion, intensity}` or writes a second memory path. **No paid CI.**
- **v1.6.3 — Realtime-as-brain (mini only).** The session `instructions` come from the core prompt builder (refreshed via `session.update` on prompt change — mood flip, memory update); the model answers **directly in audio** (the v1.6.2 shell inherits the ~6–8 s text-pipeline latency — not a live conversation; this slice is the destination); **`set_state` as a realtime tool** carries `{emotion, intensity, intent}` per spoken turn through the v0.3 gate (skipped tool → `calm`, never an error); both transcripts persist as `Message` records (the v1.5 async queue applies); a **voice-trimmed inner voice** (accent rule + the 9-intent choice + anti-mirroring traps — no verbatim `<think>` block). *DoD:* sub-second first audio; face/status update per spoken turn; memory single-path + isolation pinned; text mode untouched.
