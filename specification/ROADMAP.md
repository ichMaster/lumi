...
**DoD:** with moves on, the log shows the chosen move each exchange and **each of Лілі's messages in `store.json` carries its `move`** (one of the 7 types; `null` on user lines); streaks of 3+ mirror replies disappear over a 20-exchange mock session; types alternate per the rules; the emergency mode forces an external-material first sentence; the `move` field never alters the emotion contract (garbled → dropped silently); an old store (no `move`) loads unchanged; tags never render to the user; **off → byte-identical** (the v0.38 tests pass verbatim).
