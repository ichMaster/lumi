...
- **Core.** Canon, per-user memory + the shared experience layer, model invocation via a thin **`LLMClient`** seam (Claude Haiku from v0.1; more models switchable from v0.22), assembly and validation of the emotion field in the reply, and the daily **mood/temperament** (from v0.6, §Mood and temperament). It is **user-scoped from v0** (a single default `owner` until v1.3) so going multi-user is additive, and it does not know who displays it. Exposes one contract (§Contracts).
...
- **Voice.** A shared **ElevenLabs TTS adapter** voices Лілі's replies — a **local voicer** (v0.20) then web voice output (v2.2) — and a shared **STT adapter** hears the user — a **local dictator** (v0.21) then web dictation (v2.4). Both local pieces are separate console processes coupled to the core by files.
...
- **Plans (v0.13).** At a new local **day** the today's plan is recomposed (weekly goals + carry-overs + the v0.6 mood); at a new **week**, fresh weekly/weekend intentions (unfinished carried over). A compact **state block** (Today/This week/Weekend/Mood/Unfinished) rides in the system prompt — tone, not a report.
- **Away-gap (v0.14).** At session start the core computes the gap; when non-trivial it generates N fragments (≈1/day, soft cap; a **dream** only if the gap spanned night) via a housekeeping call from **seeds** (character + plans + mood + gap + previous + an **injected** seed). A **strong mood replans** the day and mints a *plan-vs-reality* memory. Fragments surface **to the point, never a report**; `mention_aloud` is honored.
...
## Needs (motivational substrate, from v0.13)

Under the mood and the plans sits the **why** — a few core **drives** that push Лілі from *inside* (where the horoscope/biorhythms push from outside). Not a new background loop: needs are **another computed input** to the existing seams, and they **close from what she actually did**. Woven into the inner-life phases (v0.13 = exist & pull; v0.14 = close from reality). **Global to Лілі (not per-user)**; **never competence; inner, not a demand on the user.** See [NEEDS_full.md](features/NEEDS_full.md).
...
- **Long-term memory** — durable understanding of *that user*, in **two layers** (from v0.16): a **facts layer** (`LongTermFact` — names, dates, agreements, stable preferences; precision) and an **impressions layer** (`Impression` — her first-person, emotion-weighted diary entries; voice). She **speaks from impressions and pulls facts**; the fact lives inside the impression as a promotable seed. Both per-user; both injected at startup. (Before v0.16 there is only the facts layer.) See §Emotional memory.
...
## Emotional memory (impressions, from v0.16)
...
- **Session-close generator (v0.16).** Swaps the dry fact-extractor's prompt for a diary prompt, seeded by the conversation + her per-turn emotions (v0.3) + the closeness reads (v0.10, what she sensed *he* felt) + the day's mood (v0.6/v0.8). A few impressions (restraint), each with an `emotion`, a `weight`, and a fact seed. Startup injects a first-person "what I remember about you" block alongside the facts.
- **Fading & consolidation (v0.17).** `weight` decays over time (injected clock); recall ranks by `weight × recency`; a lazy consolidation pass merges similar impressions into durable **generalizations** ("he comes alive with music") — her *understanding*, not an archive. Bounded and consistent with prior entries.
...
## Semantic recall — RAG over all messages (from v0.18)
...
- **Index & search (v0.18).** Embed each message on write; **backfill** existing messages once. A **`/recall <query>`** command runs top-K cosine over the user's vectors → dated past lines.
- **Automatic RAG (v0.19).** The incoming message is the query → top-K → a compact "relevant past moments" block injected into the turn, **deduped** against the rolling window, **capped** + a **relevance floor**; error/empty degrades to no block (never blocks a turn).
...
- **Web dictation (STT, v2.4).** The same STT adapter, now in the browser: microphone → Ukrainian recognition → text in the input field. The web sibling of the v0.21 local dictator. Provider options: Deepgram Nova-3 (uk), Whisper, or ElevenLabs Scribe.
...
- **Secrets in `.env`** (gitignored): the `ANTHROPIC_API_KEY` from **v0.1** (Claude Haiku); from **v0.20** the `ELEVENLABS_API_KEY` / voice id (local voice; reused by web voice v2.2); from **v0.21** the STT key for the chosen provider (Deepgram/ElevenLabs Scribe — or none for offline Whisper); from **v0.22** the keys for any additional models in use (`OPENAI_API_KEY` / `DEEPSEEK_API_KEY` / `MINIMAX_API_KEY`); from v3.2 the web-search API key, from v3.3 the world-context provider keys (weather/wiki/news/…), and from v4 the image-generation key and ElevenLabs Music key. The core reads keys from the environment; they never live in code or the repo.
- **The model sits behind a thin `LLMClient` seam** the core depends on (never a concrete SDK; mockable in tests). **v0.1 has one backend — Anthropic Claude Haiku** (official SDK). **v0.22 adds more** behind the same seam: other Anthropic models (Opus/Sonnet/Haiku), **OpenAI** and **DeepSeek** (a shared OpenAI-compatible adapter, different base_url/key), and **MiniMax** (its API). The active model id (and provider, from v0.22) are config values, so switching is a config change, not a code change. **Structured output is implemented per provider** (Anthropic tool output; OpenAI/DeepSeek JSON-schema `response_format`; MiniMax JSON) and always passes the v0.3 validation gate.
...
```
/core        # Python: canon, per-user memory + shared experience, llm (thin LLMClient seam — Claude Haiku v0.1, more models v0.22), emotion field + validation, mood/temperament (model-based daily mood + resolution, v0.6), repository interface (user-scoped)
...
/voice       # later (v0.20+): shared TTS adapter (ElevenLabs) + local voicer (v0.20); shared STT adapter + local dictator (v0.21); reused by web voice (v2.2) / web dictation (v2.4)
