...
- **Short memory** — at the end of a session the model compresses it (one call) into **two tiers**: a **detailed** summary and a one-line **gist** (`ShortSummary{user_id, session_id, summary, gist, ts}`). From **v0.9** the prompt injects the **last N conversations in detail** (`summary`) plus the **last D local days as compact per-day digests** — each **completed day** is consolidated **once** (lazily, on session start) from that day's per-session gists into a **`DaySummary{user_id, date, summary, ts}`** of **≤4 rows**; the prompt shows those dated digests (not the raw per-session gists) — so Лілі recalls recent chats vividly and the past few days at a glance without ballooning the prompt. (Before v0.9 there is only the detailed `summary`; old records load with an empty `gist`.)
