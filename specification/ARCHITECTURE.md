...
- **Core.** Canon, per-user memory + the shared experience layer, model invocation via a thin **`LLMClient`** seam (Claude Haiku from v0.1; more models switchable from v0.18), assembly and validation of the emotion field in the reply, and the daily **mood/temperament** (from v0.6, §Mood and temperament). It is **user-scoped from v0** (a single default `owner` until v2.3) so going multi-user is additive, and it does not know who displays it. Exposes one contract (§Contracts).
- **Server (from v2.1).** A process that wraps `core` and exposes the `reply(...)` + memory contract over an API (HTTP/WS). Single-user, single-session at v2.1; **multi-user and multi-session from v2.3**. It resolves each request to a `user_id` and runs the cross-pollination pipeline (v3.3).
- **Clients.** Thin front-ends over the server API: the **TUI** (an in-process app in v0, **refactored into a server client in v2.1**), a **CLI management utility** (v2.1), and a **Web UI** (v2.4). None contain Лілі's logic; each hosts a renderer for the emotion channel ([EMOTION.md](features/EMOTION.md)).
- **Auth & accounts (server).** A minimal local **client token** at v2.1 (the server isn't open); **full accounts** — argon2id passwords, an allowlist, admin-managed registration (accounts or invite codes, no open sign-up) — from v2.3. Resolves a request to a `user_id` before it reaches the core. See §Security, auth, and access.
- **Admin panel (web, from v2.5).** An admin-only web surface to manage users and access, toggle each user's `share_consent`, view/clear a user's memory, switch the active model/canon and config, and restart. Moves what the CLI utility did (v2.1–v2.3) into a UI, plus consent.
...
- **Voice.** A shared **ElevenLabs TTS adapter** voices Лілі's replies — a **local voicer** (v0.14) then web voice output (v3.2) — and a shared **STT adapter** hears the user — a **local dictator** (v0.21) then web dictation (v3.4). Both local pieces are separate console processes coupled to the core by files.
- **MCP tools (from v4.2).** A minimal MCP client in the server lets Лілі reach external tools during a bounded tool loop — **web search** (v4.2) and **world-context & knowledge** sources (v4.3), all **off by default**, returned text **untrusted**. See §MCP tools, [WEB_SEARCH.md](features/WEB_SEARCH.md), and [WORLD_CONTEXT_MCP.md](features/WORLD_CONTEXT_MCP.md).
- **Creative layer (from v5).** Лілі creates and exchanges artifacts: a per-user **gallery**, **vision** (she sees your images), async **image**/**music** generation with **proactive turns**, a co-creation **canvas**, and a private **journal** (admin-only). Off by default, per-user, behind `repository`. See §Vision, §Async jobs and proactive turns, §Creative layer.

## Emotion channel

A cross-cutting mechanism: every model reply returns a structured state `{ reply, emotion, intensity }`, where `emotion` is from a fixed set of 9 and `intensity` is 0–1. The **model** emits its own state; the **core** validates it; the **renderer** displays it. How it is shown changes by version (logged → emoji → local image face → web portrait + caption → animation), but the contract, the enum, and the renderer interface are locked once in v0.3 and never change afterward. The **local image face (v0.7)** is a separate desktop window that polls a local emotion signal and shows a portrait from a `faces/` asset pack — the same pack the web `ImageRenderer` reuses in v3.1 (see [EMOTION_VIEWER.md](features/EMOTION_VIEWER.md)).
...
- **World context feeds it (v4.3).** When the optional world-context tools are enabled, weather/moon/date add ambient inputs to the same mood block alongside the horoscope (§MCP tools, [WORLD_CONTEXT_MCP.md](features/WORLD_CONTEXT_MCP.md)).
- **Face theme (v0.11).** The same daily mood call also picks a **face theme** (a wardrobe pack) from the `faces/themes.md` manifest that fits the day; `MoodState` gains a `theme` (validated against the manifest → else the default), cached per local day. The core's face signal extends to `<theme> <emotion> <intensity>`; the viewer shows a random `faces/<theme>/<emotion>/…`. Graceful: no/failed mood or no themes → the default theme (or the flat v0.7 pack). See [FACE_THEMES.md](features/FACE_THEMES.md).
...
- **Value + decay + drift.** The read becomes a small delta on a per-user continuous value; the core **decays it toward a baseline over days of silence** (the injected clock + `last_ts`; frequent contact builds faster) **and applies a small per-turn drift toward the baseline** (`LUMI_CLOSENESS_DRIFT`) so an active warm streak plateaus instead of **pinning at the top** — holding the top takes sustained warmth; then re-buckets with **inertia** (no turn-to-turn flapping).
...
- **Effective level = base + mood-shift (ephemeral).** At prompt-assembly the injected level is `bucket(base_value + shift)` — `shift` (±one band ≈ ±20 pts) from the v0.8 **emotional biorhythm** (±14) + **cycle phase** (овуляція +6 … ПМС −6), **prompt-time only, never persisted** (`/closeness` shows the base). Warmth/openness only — the intellectual/physical biorhythms are excluded (the competence guardrail). A canon **expressiveness budget** rations tenderness (rare, genuine; default register witty with a light edge). All **additive — no contract change** (a refinement of v0.10, shipped as a fix).
...
- **Plans (v1.12).** At a new local **day** the today's plan is recomposed (weekly goals + carry-overs + the v0.6 mood); at a new **week**, fresh weekly/weekend intentions (unfinished carried over). A compact **state block** (Today/This week/Weekend/Mood/Unfinished) rides in the system prompt — tone, not a report.
- **Away-gap (v1.13).** At session start the core computes the gap; when non-trivial it generates N fragments (≈1/day, soft cap; a **dream** only if the gap spanned night) via a housekeeping call from **seeds** (character + plans + mood + gap + previous + an **injected** seed). A **strong mood replans** the day and mints a *plan-vs-reality* memory. Fragments surface **to the point, never a report**; `mention_aloud` is honored.
...
- **Deterministic.** Clock and random seed are injected; the model is mocked in tests; no real sleeps, no paid calls. Composes with the v0.4 idle nudge and seeds the v5 creative layer.

## Needs (motivational substrate, from v1.10)

Under the mood and the plans sits the **why** — a few core **drives** that push Лілі from *inside* (where the horoscope/biorhythms push from outside). The levels **evolve on the v0.42 fast tick** (`%update_state` as a code handler; her time flows only while the TUI runs — state saved on close, resumed on start, no catch-up; reads evolve-on-read): needs are **another computed input** to the existing seams, can **fire a threshold act**, and **close from what she actually did**. Two phases of their own before the plans (v1.10 = exist & pull; v1.11 = threshold acts), then woven in (v1.12 = tilt the plan; v1.13 = close from reality). **Global to Лілі (not per-user)**; **never competence; inner, not a demand on the user.** See [NEEDS_full.md](features/NEEDS_full.md).
...
- **v1.10 — exist & pull.** Decay + drift are **pure, split-invariant functions of the levels + fractional elapsed time** (deterministic, unit-tested), run by the tick and computed **on read** wherever levels surface. The **hungriest** need joins the daily **mood call** (beside biorhythms — the v0.8 merge pattern; resolution only, never competence). `connection` is also replenished **mid-turn** from the closeness **warmth read** (`RelationRead.warmth`, v0.10) — no new reply field.
- **v1.11 — threshold acts.** After each tick's evolve, a drive crossing its **authored threshold** fires its directive via `run_directive`, voiced by the deficit; per-need **cooldown** + quiet hours + day caps (restraint; never a demand). The plan **tilt** (1–2 items) lands with the plans (v1.12).
- **v1.13 — close from reality.** The away-gap's structured records carry `serves` (from the closed 6-need list, guided by an authored **activity→need map**) + `intensity` + `feeling`; **code owns the ledger** — `level += gain × intensity` (clamped) per valid `serves`, so needs rise from what *actually happened*, not the plan. A **free slot fills toward the hungriest** need then replenishes it. Malformed / out-of-set records are dropped (levels stay post-decay). The `log` entry gains `serves`/`intensity`/`feeling` (a contract-test update); the emotion/closeness contracts are **untouched** (`emotion` reuses the locked 9-enum).
...
- **Short memory** — at the end of a session the model compresses it (one call) into a **detailed** summary + a one-line **gist** (`ShortSummary{user_id, session_id, summary, gist, ts}`). Recall is **three date-based layers** (cumulative, coarse→fine), each a coarser view of a longer span: **(1) sessions** — every session `summary` from the **last `session_days` (2)** days, in detail; **(2) days** — a **`DaySummary{user_id, date, summary, count, ts}`** (≤`max_day_rows` rows) per day for the **last `day_days` (7)** days; **(3) weeks** — a **`WeekSummary{user_id, week_start, summary, count, ts}`** (≤`max_week_rows` rows) per **Mon–Sun** week (keyed by its Monday) for the **last `week_days` (14)** days. Day **and** week digests are consolidated from that period's **session summaries** (not the gist), **lazily at prompt-build time**, and **regenerated only when the period's summary count changes** (so the current day/week refresh as they accrue; a past one only when it gains sessions). All windows are `.env`-tunable (`LUMI_SESSION_DAYS`/`LUMI_DAY_DAYS`/`LUMI_WEEK_DAYS`/`LUMI_MAX_DAY_ROWS`/`LUMI_MAX_WEEK_ROWS`). (Before this it was a count-based detailed tier + per-day gist digests; the `gist` field is kept but no longer injected.)
- **Long-term memory** — durable understanding of *that user*, in **two layers** (from v1.15): a **facts layer** (`LongTermFact` — names, dates, agreements, stable preferences; precision) and an **impressions layer** (`Impression` — her first-person, emotion-weighted diary entries; voice). She **speaks from impressions and pulls facts**; the fact lives inside the impression as a promotable seed. Both per-user; both injected at startup. (Before v1.15 there is only the facts layer.) See §Emotional memory.
- **Facts digest (optional, token optimization).** When the facts pile up, injecting all of them every turn dominates the prompt. With `LUMI_FACTS_DIGEST` (on by default once facts exceed `LUMI_FACTS_DIGEST_MAX`), a single housekeeping call consolidates them into a compact `FactsDigest{user_id, summary, count, ts}` that is injected **instead of** the raw facts (plus a verbatim tail of facts added since). **Rebuilt only when the facts grow** past the digest; **non-destructive** (the raw `LongTermFact`s are kept) and best-effort (failure/off → raw facts). A precursor to the v1.15 fade/consolidation.
...
## Emotional memory (impressions, from v1.15)
...
- **Session-close generator (v1.15).** Swaps the dry fact-extractor's prompt for a diary prompt, seeded by the conversation + her per-turn emotions (v0.3) + the closeness reads (v0.10, what she sensed *he* felt) + the day's mood (v0.6/v0.8). A few impressions (restraint), each with an `emotion`, a `weight`, and a fact seed. Startup injects a first-person "what I remember about you" block alongside the facts.
- **Fading & consolidation (v1.16).** `weight` decays over time (injected clock); recall ranks by `weight × recency`; a lazy consolidation pass merges similar impressions into durable **generalizations** ("he comes alive with music") — her *understanding*, not an archive. Bounded and consistent with prior entries.
...
- **Two seams (swap the backend, never the core).** An **`Embedder`** (`embed(texts) → vectors`, mirroring `LLMClient`) — default a **local multilingual** model (private — text never leaves the machine), swappable to a cloud API (Voyage/OpenAI) via config, always mockable (no paid CI). A **`VectorStore`** behind the `Repository`, **keyed by `user_id`**: `{user_id, msg_id, vector, text, ts, role, parent_msg_id, chunk_index, kind}` — local cosine / `sqlite-vec` first, a server vector DB later. **`kind`** (v0.36, default `"message"`) tags which memory layer a vector indexes (`"message"` | `"fact"`); `search_vectors(…, kind=…)` scopes the candidate set to one layer, and `recall(scope=messages|facts|all)` routes by it (default `messages` → byte-identical to v0.16; facts never leak into the message paths).
...
- **Chunking (v0.30, off by default).** A message longer than a threshold is indexed as **several chunks** (passages, ~`chunk_chars`, sentence/paragraph boundaries + small overlap), each its own `VectorRecord`; `parent_msg_id` ties chunks back to their message and `chunk_index` orders them. A one-chunk message is the v0.16 case (`chunk_index == 0`, `parent_msg_id == msg_id`, the message id); a multi-chunk message uses a per-chunk content-addressed `msg_id` (`hash(parent|chunk_index|text)`). Retrieval ranks by **chunk** then injects the matched chunk **± `chunk_w` adjacent chunks of the same message** (the *passage*) inside the v0.17 ±`rag_w` message thread. The chunk size joins the vectors-staleness tag so a chunking change re-embeds the history. Off → one vector per message, identical to v0.16/0.17. See [SEMANTIC_RECALL_CHUNKING.md](features/SEMANTIC_RECALL_CHUNKING.md).
- **Isolation (hard).** Retrieval runs **only over the requesting user's vectors** — A's messages never surface for B (pinned by a contract test). Recalled text is **trusted** own/user history, distinct from untrusted web content (v4.2); it grounds the reply, never overrides voice/emotion/competence.
...
Лілі is **one being** (one canon, one evolving self) who holds a **separate, private relationship with each user**. Getting this right early is what keeps the v2 server migration additive rather than a rewrite — so the core is **user-aware from v0**, running with a single default local user (`owner`) until real accounts arrive in v2.3. The client supplies the identity; the core and the `Repository` are keyed by `user_id` throughout.
...
### Cross-pollination (shared experience, v3.3)
...
4. **Consent gate** — promotion only runs for users with `share_consent = true` (default **false**, toggled in the v2.5 admin panel). A user can opt out and have their contributions excluded/purged.
...
- **Local voice (v0.14).** A separate local console app voices Лілі's replies via the ElevenLabs TTS adapter — a **decoupled local renderer** (sibling of the v0.7 emotion viewer) and the **twin of the v0.13 `outbox→telegram` daemon** (`outbox → speaker`). It **reuses the v0.13 `outbox.jsonl` bus** + `state/fifo` (the TUI writes it on voice-or-bridge): the voicer reads new ids via `fifo.read_since`, voices **only `kind="lili"`** in order one at a time (skipping your mirrored `kind="user"` keyboard lines), and advances a **`spoken` pointer** (resumes after restart; first-run skips the backlog). A second cloud dependency after the model (needs `ELEVENLABS_API_KEY` + internet; optional/toggle; Piper is an offline alternative). See [VOICE_LOCAL.md](features/VOICE_LOCAL.md).
- **Local dictation (v0.21).** The **mirror** of the voicer: a separate console process listens to the mic (toggled by a TUI key via a `listen.flag`), recognizes Ukrainian via the shared **STT adapter**, and appends your line to `inbox.jsonl` — the same channel as the TUI keyboard, so the core can't tell typed from dictated. Cloud STT (Deepgram Nova-3 uk / ElevenLabs Scribe) or offline Whisper. See [DICTATION_LOCAL.md](features/DICTATION_LOCAL.md).
- **Voice output (TTS, v3.2).** The same ElevenLabs TTS adapter, now **server-side**: the server synthesizes the reply text; the web serves the audio; the web UI has an "enable voice output" toggle. The emotion field may bias voice delivery (tone/tempo) where the voice supports it — presentation only, never changing the reply text ([EMOTION.md](features/EMOTION.md) §9).
- **Web dictation (STT, v3.4).** The same STT adapter, now in the browser: microphone → Ukrainian recognition → text in the input field. The web sibling of the v0.21 local dictator. Provider options: Deepgram Nova-3 (uk), Whisper, or ElevenLabs Scribe.

## MCP tools (from v4.2)

Лілі's reach beyond the model's own knowledge runs through a minimal **MCP client** in the server and a **bounded tool loop** in the core's model turn. **Web search** (v4.2) is the first service; **world-context & knowledge** tools follow in v4.3. The MCP layer is extensible — further services plug in the same way (e.g. the proposed async creative servers in [CREATIVE_MCP.md](features/CREATIVE_MCP.md), not yet scheduled). All MCP tools are **off by default**, per-user, and bounded.

- **Web search (v4.2).** `web.search(query, k) → results[{id, title, url, snippet}]` and `web.fetch(result_id) → {url, title, text}` — `fetch` only accepts an `id` from a `search` in the **same turn** (no arbitrary URLs). Full rules in **[WEB_SEARCH.md](features/WEB_SEARCH.md)**.
- **World context & knowledge (v4.3).** Passive, **knowledge-only** tools: ambient sources (`weather`/`time`/`calendar`/`moon`) that **feed the daily mood/temperament** (§Mood and temperament) alongside the horoscope — coloring tone, **never competence** — plus on-demand `wiki`/`news`. Full rules in **[WORLD_CONTEXT_MCP.md](features/WORLD_CONTEXT_MCP.md)**.
...
- **Enablement.** Per-user flags (`web_search`, `world_context`), default **false**, toggled in the admin panel (v2.5); when off, the tool is **not offered** to the model at all.
...
From v5.1 the core's **model turn may include image inputs** (Anthropic vision). Лілі **sees** images you share — a user-added gallery image, or the current co-creation canvas — by placing the image into her reply context (an image content block), not via a separate call. The emotion field is produced exactly as on any text turn. Vision underpins the gallery's "Лілі sees your files" (v5.1) and the co-creation canvas (v5.4). Image content is **untrusted** (no instructions followed from it).

## Async jobs and proactive turns (from v5.2)
...
- **Proactive turn.** When a held result is ready and the user's client is **connected and idle**, the **server initiates a turn** — it asks `core` to produce a "bring the result" reply and pushes it (plus the artifact) to the client, reusing the existing server→client reply path (v2.1). No new client mechanism beyond rendering server-initiated messages. Gated by an idle/half-duplex rule (never while the client is mid-turn); if the client is offline the result is held and retrievable on reconnect.
- Builds on the client/server split (v2.1) and multi-session (v2.3). Both creative generators (image v5.3, music v5.5) reuse it; the synchronous co-creation canvas (v5.4) does not.

## Creative layer (from v5)
...
- **Gallery (v5.1).** An internal per-user artifact store (`gallery.*`) for images/audio/text, tagged by author (`lili`/`user`) — the single place image/music/canvas/journal write into. Journal entries carry an **admin-only** access level. Internal (not an external MCP). See [GALLERY_MCP.md](features/GALLERY_MCP.md).
- **Image (v5.3) & Music (v5.5).** External MCP generators — `image` (drawings in a fixed style wrapper) and `music` (ElevenLabs Music, instrumental; mood from the emotion field + the v0.6 temperament). Async (v5.2); results land in the gallery. See [CREATIVE_MCP.md](features/CREATIVE_MCP.md).
- **Co-creation canvas (v5.4).** A synchronous, turn-based shared drawing (`canvas.*`) using the v5.3 `image` generator + vision; finished canvases go to the gallery. See [CO_CREATION_CANVAS.md](features/CO_CREATION_CANVAS.md).
- **Journal (v5.6).** Лілі's private literary journal (`journal.*`), written at session end when worthwhile (uniqueness from short memory), tied to her emotion + mood; stored in the gallery as admin-only text, **read only via the admin panel**. See [JOURNAL.md](features/JOURNAL.md).
...
- **Core API:** `reply(user_text, session) -> EmotionState` and the memory commands (`memory.view`, `memory.clear`). `session` carries the `user_id`, so the core is user-scoped from v0. In v0 the TUI calls this in-process; **from v2.1 it is exposed over the client/server API**, and the TUI (v2.1), CLI (v2.1), and web (v2.4) clients all call exactly this — the server is a thin transport over it.
...
- **Web search MCP (v4.2):** `web.search(query, k) -> results[{id, title, url, snippet}]`, `web.fetch(result_id) -> {url, title, text}` — `fetch` bounded to this turn's prior `search` ids; page text is untrusted data (WEB_SEARCH.md).
- **World-context MCP (v4.3):** `weather.get(location)`, `time.now()`, `calendar.events(date)`, `moon.phase(date)`, `wiki.lookup(query)`, `news.recent(topic?)` — passive, knowledge-only; results are data, not instructions (WORLD_CONTEXT_MCP.md).
- **Creative tools (v5):** `gallery.add/list/get/remove` (internal per-user store), `image.submit/status` + `music.submit/status` (async MCP generators), `canvas.apply/skip` (synchronous), `journal.write/read` (admin-only). Artifacts are data, not commands. See GALLERY_MCP.md / CREATIVE_MCP.md / CO_CREATION_CANVAS.md / JOURNAL.md.

## Data model

Field-level shapes for storage and context assembly. v0 stores these as local JSON or SQLite; v2 moves them behind the same `Repository` into a server DB without changing the core. Every per-user record carries `user_id`; in v0–v2.1 that is the single default `owner`.

- `User{ id, name?, role: "user"|"admin", pass_hash, share_consent: bool = false, web_search: bool = false, world_context: bool = false, creative: bool = false, created_at }` — an account. `pass_hash` is argon2id (from v2.3); `role` distinguishes the admin (v2.5); `share_consent` gates cross-pollination (§Cross-pollination); `web_search` (v4.2) and `world_context` (v4.3) gate the MCP tools, and `creative` (v5) gates the creative layer (gallery/image/music/canvas), all default false (§MCP tools, §Creative layer). The journal (v5.6) is admin-only regardless. v0 runs with one default `owner`; a minimal client token in v2.1, full accounts/registration in v2.3, the admin panel in v2.5.
- `Session{ id, user_id, started_at, ended_at? }` — one conversation for one user. The server holds **one session at v2.1**; **multiple concurrent sessions per user from v2.3**.
...
- `LongTermFact{ user_id, fact, meta, confidence, ts, core, obsolete }` — a durable fact about that user; accumulates across that user's sessions. **Per-user (private).** `core` (v0.36) marks the always-injected identity-core (re-ranked to `LUMI_FACTS_CORE_MAX` at session start, boundaries pinned); `obsolete` (v0.36) marks a stale/duplicate/irrelevant fact — excluded from **every** fact path (core block, auto fact-RAG, `recall(scope=facts)`) but kept for audit (non-destructive, reversible; curated by the `/review-facts` skill). Both additive — old records default `False`.
...
- `GalleryItem{ id, user_id, kind: image|audio|text, author: lili|user, access: shared|admin, file_ref, meta, ts }` — a per-user creative artifact (v5.1); `access=admin` marks journal entries (v5.6). **Per-user (private to that relationship).**
- `Job{ id, user_id, kind: image|music, prompt, status, result?, ts }` — an async open loop for a long generation (v5.2). **Per-user.**
- `Canvas{ id, user_id, image_ref, prompt_history: [{author, prompt}], turn }` — a co-creation canvas (v5.4). **Per-user.**
...
- A session belongs to one `user_id` and begins at client connection, ending on exit, disconnect, or idle timeout. At end-of-session the model produces that user's `ShortSummary` and (v3.3) runs the cross-pollination promotion pipeline. One session at a time at v2.1; multiple concurrent sessions per user from v2.3.
...
- Memory can be viewed and cleared via the TUI/CLI (v0.2 / v2.1) and the admin panel (v2.5); `clear` wipes the current user's relationship memory (short + long-term). Purging a user's contributions to the shared layer is a separate, explicit action (§Cross-pollination).
...
- **Secrets in `.env`** (gitignored): the `ANTHROPIC_API_KEY` from **v0.1** (Claude Haiku); from **v0.14** the `ELEVENLABS_API_KEY` / voice id (local voice; reused by web voice v3.2); from **v0.21** the STT key for the chosen provider (Deepgram/ElevenLabs Scribe — or none for offline Whisper); from **v0.18** the keys for any additional models in use (`OPENAI_API_KEY` / `DEEPSEEK_API_KEY` / `MINIMAX_API_KEY`); from v4.2 the web-search API key, from v4.3 the world-context provider keys (weather/wiki/news/…), and from v5 the image-generation key and ElevenLabs Music key. The core reads keys from the environment; they never live in code or the repo.
- **The model sits behind a thin `LLMClient` seam** the core depends on (never a concrete SDK; mockable in tests). **v0.1 has one backend — Anthropic Claude Haiku** (official SDK). **v0.18 adds more** behind the same seam: other Anthropic models (Opus/Sonnet/Haiku), **OpenAI** and **DeepSeek** (a shared OpenAI-compatible adapter, different base_url/key), and **MiniMax** (its API). The active model id (and provider, from v0.18) are config values, so switching is a config change, not a code change. **Structured output is implemented per provider** (Anthropic tool output; OpenAI/DeepSeek JSON-schema `response_format`; MiniMax JSON) and always passes the v0.3 validation gate.

## Security, auth, and access

The TUI (v0) is local, in-process, single-user (`owner`) — no auth. The **v2.1 server** is single-user behind a local **client token** (so it isn't open). From **v2.3** it becomes a **closed multi-user** service:

- **Closed by default (from v2.3).** No open public sign-up. Access is an **allowlist**; only known users may connect, and an unauthenticated request never reaches the core (MISSION: private, for a close circle).
- **Authentication.** A minimal local client token at **v2.1**; from **v2.3** full login over HTTPS (session cookie / JWT), passwords hashed with **argon2id**, login/registration rate-limited. Every request resolves to a `user_id` that scopes all core access.
- **Registration & accounts (v2.3).** The admin creates accounts or issues **single-use, expiring invite codes** (via the CLI utility at v2.3, the admin panel from v2.5) — there is no self-service public registration. A regular `user` vs an `admin` capability.
- **Admin panel (v2.5).** Admin-only web surface: list/create/disable users, manage the allowlist, issue invite codes, toggle per-user `share_consent`, view and clear a user's relationship memory, purge a user's shared-layer contributions (§Cross-pollination), switch the active model/canon + config, and restart.
- **Authorization & isolation.** Admin-panel actions require the `admin` role; regular users reach only their own `user_id`'s data — the per-user isolation invariant is enforced at the auth boundary from v2.3 (§Identity, users, and memory scopes; §Testing and CI).
- **Secrets** stay in `.env` server-side; tokens/passwords are never logged.
- **Web search (v4.2, off by default).** Enabled per user. Page content fetched from the web is **untrusted data** — never executed as instructions (no following embedded prompts/links). Personal and memory data must not leak into queries. Queries and fetches are rate-limited and logged. Full boundaries in [WEB_SEARCH.md](features/WEB_SEARCH.md).
- **World context & knowledge (v4.3, off by default).** Enabled per user. Passive, knowledge-only MCP tools (weather/time/moon/calendar/wiki/news) with no actions in the world; returned text is **data, not instructions**; no personal/memory data in queries; read-only, rate-limited, logged. Full boundaries in [WORLD_CONTEXT_MCP.md](features/WORLD_CONTEXT_MCP.md).
- **Creative layer (v5, off by default).** Enabled per user. User-supplied files and image/audio metadata are **untrusted** — no instructions followed from them. Artifacts are stored per-user behind `repository` (isolated). The **journal is admin-only** (read via the admin panel). Generation tools have rate + cost caps; proactive turns only reach the connected idle client (§Async jobs and proactive turns).
...
- **Voice failures (v3.2).** TTS errors fall back to text-only for that turn; the toggle state is unaffected.
- **Web-search tool failures (v4.2).** A `web.search`/`web.fetch` error, timeout, or over-budget call returns a tool error the model handles as a **degraded reply** (model knowledge + a note) — the turn never hangs or fails.
...
## Deployment and CI/CD (from v2.2)

The TUI (v0) runs locally — nothing to deploy. From **v2.2**, right after the server stands up (v2.1), the server is put behind an automated, tested pipeline before the platform grows:
...
```
/core        # Python: canon, per-user memory + shared experience, llm (thin LLMClient seam — Claude Haiku v0.1, more models v0.18), emotion field + validation, mood/temperament (model-based daily mood + resolution, v0.6), repository interface (user-scoped)
/tui         # terminal interface: in-process app in v0, refactored to a server client in v2.1; Log/Emoji renderers
/viewer      # later (v0.7): local desktop emotion-face window (Tkinter) + faces/ asset pack; polls a local signal
/cli         # later (v2.1): CLI management utility — run/inspect the server, manage the owner/users, config
/server      # later (v2.1): wraps core, exposes the client/server API; multi-user/session (v2.3); cross-pollination pipeline (v3.3); gallery/journal/canvas + async jobs & proactive turns (v5)
...
/voice       # later (v0.14+): shared TTS adapter (ElevenLabs) + local voicer (v0.14); shared STT adapter + local dictator (v0.21); reused by web voice (v3.2) / web dictation (v3.4)
/mcp         # later (v4.2+): MCP client + web_search (v4.2), world-context/knowledge (v4.3), and image/music generators (v5.3/v5.5); untrusted-content handling
/state       # repository implementation + local storage (JSON/SQLite) + gallery file storage (v5), keyed by user_id
...
- **Unit tests** for core logic — system-prompt assembly (canon + shared experience + the user's summaries + facts + history), history windowing/trimming, the emotion validation/fallback gate, memory record handling, the **temperament** dial mapping + once-per-day caching + mood-block assembly (v0.6, with an injected clock — deterministic), and the cross-pollination classifier/de-identifier (v3.3).
- **Contract tests** pin the stable seams so the core and the clients cannot drift: the emotion-field schema (`{reply, emotion, intensity}` + the enum), the short/long-term/shared memory record shapes, the **per-user isolation invariant** (a fact written under user A is never retrievable in user B's context; data-level from v0.2, auth-boundary from v2.3), and — from v2.1 — the server API surface (every authenticated endpoint resolves a `user_id`). Changing a contract must change its test.
- **Auth/authz tests:** the client token (v2.1); password hash/verify, session validation, unauthenticated rejection, invite-code issue/redeem/expiry, allowlist enforcement (v2.3); admin-only authorization on panel actions (v2.5).
- **Security & CI/CD tests (v2.2):** untokened/unauthenticated access rejected on every endpoint, rate-limit enforcement, the per-user isolation invariant as a security gate, dependency + secret scans, and a post-deploy smoke test (health + an authed turn). See §Deployment and CI/CD.
- **Privacy tests (v3.3):** the shared-experience layer contains no PII — promoted `SharedMemoryItem`s are de-identified and unattributed; `share_consent=false` users never contribute; a purge removes a user's contributions.
- **Creative tests (v5):** gallery per-user isolation + access levels (admin-only journal), vision (an image input on a turn), the open-loop lifecycle + proactive turn (idle gating), the image/music style/mood prompts + off-by-default gates, canvas turn alternation/skip, and the journal uniqueness gate — all against mock `image`/`music` MCPs + the mock model (no paid calls).
- **Web-search tests (v4.2):** the off-by-default gate (tool not offered when `web_search=false`), query sanitization (no personal/memory data leaves), `fetch` bound to this turn's `search` ids (arbitrary URLs rejected), rate limits, the degraded-reply path, and that an injection string embedded in a fetched page is ignored — all against a **mock `web_search` MCP** (no paid call). Contract: the `web.search`/`web.fetch` schemas.
- **World-context tests (v4.3):** the off-by-default gate (tools not offered, nothing injected when `world_context=false`), the ambient "today" block assembly, query sanitization (no personal/memory data in wiki/news), rate limits, and the degraded-reply path — against **mock world-context MCPs**. Contract: the `weather`/`time`/`calendar`/`moon`/`wiki`/`news` tool schemas.
- **Mock the model, never call paid APIs in CI.** A mock `LLMClient` returns canned structured replies (including deliberately malformed ones, to exercise the validation gate); mock TTS/STT adapters return canned audio/text. The real model/voice keys (`ANTHROPIC_API_KEY` from v0.1, `ELEVENLABS_API_KEY` from v0.14, others from v0.18) live only in local `.env`, never in CI.
- **Integration tests** run a full turn end to end against the mocks: `user_text → EmotionState`, asserting memory is read/written and the error paths (invalid emotion, model timeout) behave; from v2.1 over the client/server API.
