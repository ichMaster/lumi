...
- **Plans (v1.3).** At a new local **day** the today's plan is recomposed (weekly goals + carry-overs + the v0.6 mood); at a new **week**, fresh weekly/weekend intentions (unfinished carried over). A compact **state block** (Today/This week/Weekend/Mood/Unfinished) rides in the system prompt — tone, not a report.
- **Away-gap (v1.4).** At session start the core computes the gap; when non-trivial it generates N fragments (≈1/day, soft cap; a **dream** only if the gap spanned night) via a housekeeping call from **seeds** (character + plans + mood + gap + previous + an **injected** seed). A **strong mood replans** the day and mints a *plan-vs-reality* memory. Fragments surface **to the point, never a report**; `mention_aloud` is honored.
...
## Needs (motivational substrate, from v1.1)

Under the mood and the plans sits the **why** — a few core **drives** that push Лілі from *inside* (where the horoscope/biorhythms push from outside). The levels **evolve on the v0.43 fast tick** (`%update_state` as a code handler; her time flows only while the TUI runs — state saved on close, resumed on start, no catch-up; reads evolve-on-read): needs are **another computed input** to the existing seams, can **fire a threshold act**, and **close from what she actually did**. Two phases of their own before the plans (v1.1 = exist & pull; v1.2 = threshold acts), then woven in (v1.3 = tilt the plan; v1.4 = close from reality). **Global to Лілі (not per-user)**; **never competence; inner, not a demand on the user.** See [NEEDS_full.md](features/NEEDS_full.md).
...
- **Long-term memory** — durable understanding of *that user*, in **two layers** (from v1.6): a **facts layer** (`LongTermFact` — names, dates, agreements, stable preferences; precision) and an **impressions layer** (`Impression` — her first-person, emotion-weighted diary entries; voice). She **speaks from impressions and pulls facts**; the fact lives inside the impression as a promotable seed. Both per-user; both injected at startup. (Before v1.6 there is only the facts layer.) See §Emotional memory.
- **Facts digest (optional, token optimization).** When the facts pile up, injecting all of them every turn dominates the prompt. With `LUMI_FACTS_DIGEST` (on by default once facts exceed `LUMI_FACTS_DIGEST_MAX`), a single housekeeping call consolidates them into a compact `FactsDigest{user_id, summary, count, ts}` that is injected **instead of** the raw facts (plus a verbatim tail of facts added since). **Rebuilt only when the facts grow** past the digest; **non-destructive** (the raw `LongTermFact`s are kept) and best-effort (failure/off → raw facts). A precursor to the v1.6 fade/consolidation.
...
## Emotional memory (impressions, from v1.6)
...
- **Session-close generator (v1.6).** Swaps the dry fact-extractor's prompt for a diary prompt, seeded by the conversation + her per-turn emotions (v0.3) + the closeness reads (v0.10, what she sensed *he* felt) + the day's mood (v0.6/v0.8). A few impressions (restraint), each with an `emotion`, a `weight`, and a fact seed. Startup injects a first-person "what I remember about you" block alongside the facts.
- **Fading & consolidation (v1.7).** `weight` decays over time (injected clock); recall ranks by `weight × recency`; a lazy consolidation pass merges similar impressions into durable **generalizations** ("he comes alive with music") — her *understanding*, not an archive. Bounded and consistent with prior entries.
