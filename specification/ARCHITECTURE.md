...
- **Plans (v0.15).** At a new local **day** the today's plan is recomposed (weekly goals + carry-overs + the v0.6 mood); at a new **week**, fresh weekly/weekend intentions (unfinished carried over). A compact **state block** (Today/This week/Weekend/Mood/Unfinished) rides in the system prompt — tone, not a report.
- **Away-gap (v0.16).** At session start the core computes the gap; when non-trivial it generates N fragments (≈1/day, soft cap; a **dream** only if the gap spanned night) via a housekeeping call from **seeds** (character + plans + mood + gap + previous + an **injected** seed). A **strong mood replans** the day and mints a *plan-vs-reality* memory. Fragments surface **to the point, never a report**; `mention_aloud` is honored.
...
## Needs (motivational substrate, from v0.15)

Under the mood and the plans sits the **why** — a few core **drives** that push Лілі from *inside* (where the horoscope/biorhythms push from outside). Not a new background loop: needs are **another computed input** to the existing seams, and they **close from what she actually did**. Woven into the inner-life phases (v0.15 = exist & pull; v0.16 = close from reality). **Global to Лілі (not per-user)**; **never competence; inner, not a demand on the user.** See [NEEDS_full.md](features/NEEDS_full.md).
...
- **Long-term memory** — durable understanding of *that user*, in **two layers** (from v0.18): a **facts layer** (`LongTermFact` — names, dates, agreements, stable preferences; precision) and an **impressions layer** (`Impression` — her first-person, emotion-weighted diary entries; voice). She **speaks from impressions and pulls facts**; the fact lives inside the impression as a promotable seed. Both per-user; both injected at startup. (Before v0.18 there is only the facts layer.) See §Emotional memory.
...
## Emotional memory (impressions, from v0.18)
...
- **Session-close generator (v0.18).** Swaps the dry fact-extractor's prompt for a diary prompt, seeded by the conversation + her per-turn emotions (v0.3) + the closeness reads (v0.10, what she sensed *he* felt) + the day's mood (v0.6/v0.8). A few impressions (restraint), each with an `emotion`, a `weight`, and a fact seed. Startup injects a first-person "what I remember about you" block alongside the facts.
- **Fading & consolidation (v0.19).** `weight` decays over time (injected clock); recall ranks by `weight × recency`; a lazy consolidation pass merges similar impressions into durable **generalizations** ("he comes alive with music") — her *understanding*, not an archive. Bounded and consistent with prior entries.
...
## Semantic recall — RAG over all messages (from v0.20)
...
- **Index & search (v0.20).** Embed each message on write; **backfill** existing messages once. A **`/recall <query>`** command runs top-K cosine over the user's vectors → dated past lines.
- **Automatic RAG (v0.21).** The incoming message is the query → top-K → a compact "relevant past moments" block injected into the turn, **deduped** against the rolling window, **capped** + a **relevance floor**; error/empty degrades to no block (never blocks a turn).
