...
- **Core.** Canon, per-user memory + the shared experience layer, model invocation via a thin **`LLMClient`** seam (Claude Haiku from v0.1; more models switchable from v0.9), assembly and validation of the emotion field in the reply, and the daily **mood/temperament** (from v0.8, §Mood and temperament). It is **user-scoped from v0** (a single default `owner` until v1.3) so going multi-user is additive, and it does not know who displays it. Exposes one contract (§Contracts).
...
- **Voice.** A shared **ElevenLabs TTS adapter** voices Лілі's replies — a **local voicer** (v0.6) then web voice output (v2.2) — and a shared **STT adapter** hears the user — a **local dictator** (v0.7) then web dictation (v2.4). Both local pieces are separate console processes coupled to the core by files.
...
- **Web dictation (STT, v2.4).** The same STT adapter, now in the browser: microphone → Ukrainian recognition → text in the input field. The web sibling of the v0.7 local dictator. Provider options: Deepgram Nova-3 (uk), Whisper, or ElevenLabs Scribe.
...
- **Image (v4.3) & Music (v4.5).** External MCP generators — `image` (drawings in a fixed style wrapper) and `music` (ElevenLabs Music, instrumental; mood from the emotion field + the v0.8 temperament). Async (v4.2); results land in the gallery. See [CREATIVE_MCP.md](features/CREATIVE_MCP.md).
...
- `NatalChart` — a fixed JSON snapshot (timestamp + geo + computed positions) for Лілі, written once; part of canon/config (one Лілі, shared); drives the daily temperament (§Mood and temperament, from v0.8).
...
- Session history lives in RAM during the session and is trimmed to a rolling window (last N turns or a token budget) before each model call; older turns are folded into the short summary rather than sent verbatim.
...
- **Config is explicit and switchable.** Model selection (Opus / Sonnet / Haiku), the active canon, memory window size, and renderer/client options live in config — not hardcoded in the core.
- **Secrets in `.env`** (gitignored): the `ANTHROPIC_API_KEY` from **v0.1** (Claude Haiku); from **v0.6** the `ELEVENLABS_API_KEY` / voice id (local voice; reused by web voice v2.2); from **v0.7** the STT key for the chosen provider (Deepgram/ElevenLabs Scribe — or none for offline Whisper); from **v0.9** the keys for any additional models in use (`OPENAI_API_KEY` / `DEEPSEEK_API_KEY` / `MINIMAX_API_KEY`); from v3.2 the web-search API key, from v3.3 the world-context provider keys (weather/wiki/news/…), and from v4 the image-generation key and ElevenLabs Music key. The core reads keys from the environment; they never live in code or the repo.
...
```
/core        # Python: canon, per-user memory + shared experience, llm (thin LLMClient seam — Claude Haiku v0.1, more models v0.9), emotion field + validation, mood/temperament (astro engine — skyfield, v0.8), repository interface (user-scoped)
/tui         # terminal interface: in-process app in v0, refactored to a server client in v1.1; Log/Emoji renderers
/viewer      # later (v0.5): local desktop emotion-face window (Tkinter) + faces/ asset pack; polls a local signal
...
/voice       # later (v0.6+): shared TTS adapter (ElevenLabs) + local voicer (v0.6); shared STT adapter + local dictator (v0.7); reused by web voice (v2.2) / web dictation (v2.4)
