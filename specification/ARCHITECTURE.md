...
- **Core.** Canon, per-user memory + the shared experience layer, model invocation via a thin **`LLMClient`** seam (Claude Haiku from v0.1; more models switchable from v0.10), assembly and validation of the emotion field in the reply, and the daily **mood/temperament** (from v0.6, §Mood and temperament). It is **user-scoped from v0** (a single default `owner` until v1.3) so going multi-user is additive, and it does not know who displays it. Exposes one contract (§Contracts).
...
- **Voice.** A shared **ElevenLabs TTS adapter** voices Лілі's replies — a **local voicer** (v0.8) then web voice output (v2.2) — and a shared **STT adapter** hears the user — a **local dictator** (v0.9) then web dictation (v2.4). Both local pieces are separate console processes coupled to the core by files.
...
- **Web dictation (STT, v2.4).** The same STT adapter, now in the browser: microphone → Ukrainian recognition → text in the input field. The web sibling of the v0.9 local dictator. Provider options: Deepgram Nova-3 (uk), Whisper, or ElevenLabs Scribe.
...
- **Secrets in `.env`** (gitignored): the `ANTHROPIC_API_KEY` from **v0.1** (Claude Haiku); from **v0.8** the `ELEVENLABS_API_KEY` / voice id (local voice; reused by web voice v2.2); from **v0.9** the STT key for the chosen provider (Deepgram/ElevenLabs Scribe — or none for offline Whisper); from **v0.10** the keys for any additional models in use (`OPENAI_API_KEY` / `DEEPSEEK_API_KEY` / `MINIMAX_API_KEY`); from v3.2 the web-search API key, from v3.3 the world-context provider keys (weather/wiki/news/…), and from v4 the image-generation key and ElevenLabs Music key. The core reads keys from the environment; they never live in code or the repo.
- **The model sits behind a thin `LLMClient` seam** the core depends on (never a concrete SDK; mockable in tests). **v0.1 has one backend — Anthropic Claude Haiku** (official SDK). **v0.10 adds more** behind the same seam: other Anthropic models (Opus/Sonnet/Haiku), **OpenAI** and **DeepSeek** (a shared OpenAI-compatible adapter, different base_url/key), and **MiniMax** (its API). The active model id (and provider, from v0.10) are config values, so switching is a config change, not a code change. **Structured output is implemented per provider** (Anthropic tool output; OpenAI/DeepSeek JSON-schema `response_format`; MiniMax JSON) and always passes the v0.3 validation gate.
...
/voice       # later (v0.8+): shared TTS adapter (ElevenLabs) + local voicer (v0.8); shared STT adapter + local dictator (v0.9); reused by web voice (v2.2) / web dictation (v2.4)
