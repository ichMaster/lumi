...
- **Core.** Canon, per-user memory + the shared experience layer, model invocation via a thin **`LLMClient`** seam (Claude Haiku from v0.1; more models switchable from v0.18), assembly and validation of the emotion field in the reply, and the daily **mood/temperament** (from v0.6, §Mood and temperament). It is **user-scoped from v0** (a single default `owner` until v3.3) so going multi-user is additive, and it does not know who displays it. Exposes one contract (§Contracts).
- **Server (from v3.1).** A process that wraps `core` and exposes the `reply(...)` + memory contract over an API (HTTP/WS). Single-user, single-session at v3.1; **multi-user and multi-session from v3.3**. It resolves each request to a `user_id` and runs the cross-pollination pipeline (v4.3).
- **Clients.** Thin front-ends over the server API: the **TUI** (an in-process app in v0, **refactored into a server client in v3.1**), a **CLI management utility** (v3.1), and a **Web UI** (v3.4). None contain Лілі's logic; each hosts a renderer for the emotion channel ([EMOTION.md](features/EMOTION.md)).
- **Auth & accounts (server).** A minimal local **client token** at v3.1 (the server isn't open); **full accounts** — argon2id passwords, an allowlist, admin-managed registration (accounts or invite codes, no open sign-up) — from v3.3. Resolves a request to a `user_id` before it reaches the core. See §Security, auth, and access.
- **Admin panel (web, from v3.5).** An admin-only web surface to manage users and access, toggle each user's `share_consent`, view/clear a user's memory, switch the active model/canon and config, and restart. Moves what the CLI utility did (v3.1–v3.3) into a UI, plus consent.
...
- **Voice.** A shared **ElevenLabs TTS adapter** voices Лілі's replies — a **local voicer** (v0.14) then web voice output (v4.2) — and a shared **STT adapter** hears the user — a **local dictator** (v0.21) then web dictation (v4.4). Both local pieces are separate console processes coupled to the core by files.
- **MCP tools (from v5.2).** A minimal MCP client in the server lets Лілі reach external tools during a bounded tool loop — **web search** (v5.2) and **world-context & knowledge** sources (v5.3), all **off by default**, returned text **untrusted**. See §MCP tools, [WEB_SEARCH.md](features/WEB_SEARCH.md), and [WORLD_CONTEXT_MCP.md](features/WORLD_CONTEXT_MCP.md).
- **Creative layer (from v6).** Лілі creates and exchanges artifacts: a per-user **gallery**, **vision** (she sees your images), async **image**/**music** generation with **proactive turns**, a co-creation **canvas**, and a private **journal** (admin-only). Off by default, per-user, behind `repository`. See §Vision, §Async jobs and proactive turns, §Creative layer.

## Emotion channel

A cross-cutting mechanism: every model reply returns a structured state `{ reply, emotion, intensity }`, where `emotion` is from a fixed set of 9 and `intensity` is 0–1. The **model** emits its own state; the **core** validates it; the **renderer** displays it. How it is shown changes by version (logged → emoji → local image face → web portrait + caption → animation), but the contract, the enum, and the renderer interface are locked once in v0.3 and never change afterward. The **local image face (v0.7)** is a separate desktop window that polls a local emotion signal and shows a portrait from a `faces/` asset pack — the same pack the web `ImageRenderer` reuses in v4.1 (see [EMOTION_VIEWER.md](features/EMOTION_VIEWER.md)).
...
- **World context feeds it (v5.3).** When the optional world-context tools are enabled, weather/moon/date add ambient inputs to the same mood block alongside the horoscope (§MCP tools, [WORLD_CONTEXT_MCP.md](features/WORLD_CONTEXT_MCP.md)).
...
- **Plans (v2.4).** At a new local **day** the today's plan is recomposed (weekly goals + carry-overs + the v0.6 mood); at a new **week**, fresh weekly/weekend intentions (unfinished carried over). A compact **state block** (Today/This week/Weekend/Mood/Unfinished) rides in the system prompt — tone, not a report.
- **Away-gap (v2.5).** At session start the core computes the gap; when non-trivial it generates N fragments (≈1/day, soft cap; a **dream** only if the gap spanned night) via a housekeeping call from **seeds** (character + plans + mood + gap + previous + an **injected** seed). A **strong mood replans** the day and mints a *plan-vs-reality* memory. Fragments surface **to the point, never a report**; `mention_aloud` is honored.
...
- **Deterministic.** Clock and random seed are injected; the model is mocked in tests; no real sleeps, no paid calls. Composes with the v0.4 idle nudge and seeds the v6 creative layer.

## Needs (motivational substrate, from v2.2)

Under the mood and the plans sits the **why** — a few core **drives** that push Лілі from *inside* (where the horoscope/biorhythms push from outside). The levels **evolve on the v0.42 fast tick** (`%update_state` as a code handler; her time flows only while the TUI runs — state saved on close, resumed on start, no catch-up; reads evolve-on-read): needs are **another computed input** to the existing seams, can **fire a threshold act**, and **close from what she actually did**. Two phases of their own before the plans (v2.2 = exist & pull; v2.3 = threshold acts), then woven in (v2.4 = tilt the plan; v2.5 = close from reality). **Global to Лілі (not per-user)**; **never competence; inner, not a demand on the user.** See [NEEDS_full.md](features/NEEDS_full.md).
...
- **Long-term memory** — durable understanding of *that user*, in **two layers** (from v2.7): a **facts layer** (`LongTermFact` — names, dates, agreements, stable preferences; precision) and an **impressions layer** (`Impression` — her first-person, emotion-weighted diary entries; voice). She **speaks from impressions and pulls facts**; the fact lives inside the impression as a promotable seed. Both per-user; both injected at startup. (Before v2.7 there is only the facts layer.) See §Emotional memory.
- **Facts digest (optional, token optimization).** When the facts pile up, injecting all of them every turn dominates the prompt. With `LUMI_FACTS_DIGEST` (on by default once facts exceed `LUMI_FACTS_DIGEST_MAX`), a single housekeeping call consolidates them into a compact `FactsDigest{user_id, summary, count, ts}` that is injected **instead of** the raw facts (plus a verbatim tail of facts added since). **Rebuilt only when the facts grow** past the digest; **non-destructive** (the raw `LongTermFact`s are kept) and best-effort (failure/off → raw facts). A precursor to the v2.7 fade/consolidation.
...
## Emotional memory (impressions, from v2.7)
...
- **Session-close generator (v2.7).** Swaps the dry fact-extractor's prompt for a diary prompt, seeded by the conversation + her per-turn emotions (v0.3) + the closeness reads (v0.10, what she sensed *he* felt) + the day's mood (v0.6/v0.8). A few impressions (restraint), each with an `emotion`, a `weight`, and a fact seed. Startup injects a first-person "what I remember about you" block alongside the facts.
- **Fading & consolidation (v2.8).** `weight` decays over time (injected clock); recall ranks by `weight × recency`; a lazy consolidation pass merges similar impressions into durable **generalizations** ("he comes alive with music") — her *understanding*, not an archive. Bounded and consistent with prior entries.
...
- **Isolation (hard).** Retrieval runs **only over the requesting user's vectors** — A's messages never surface for B (pinned by a contract test). Recalled text is **trusted** own/user history, distinct from untrusted web content (v5.2); it grounds the reply, never overrides voice/emotion/competence.
...
Лілі is **one being** (one canon, one evolving self) who holds a **separate, private relationship with each user**. Getting this right early is what keeps the v3 server migration additive rather than a rewrite — so the core is **user-aware from v0**, running with a single default local user (`owner`) until real accounts arrive in v3.3. The client supplies the identity; the core and the `Repository` are keyed by `user_id` throughout.
...
### Cross-pollination (shared experience, v4.3)
...
4. **Consent gate** — promotion only runs for users with `share_consent = true` (default **false**, toggled in the v3.5 admin panel). A user can opt out and have their contributions excluded/purged.
...
- **Voice output (TTS, v4.2).** The same ElevenLabs TTS adapter, now **server-side**: the server synthesizes the reply text; the web serves the audio; the web UI has an "enable voice output" toggle. The emotion field may bias voice delivery (tone/tempo) where the voice supports it — presentation only, never changing the reply text ([EMOTION.md](features/EMOTION.md) §9).
- **Web dictation (STT, v4.4).** The same STT adapter, now in the browser: microphone → Ukrainian recognition → text in the input field. The web sibling of the v0.21 local dictator. Provider options: Deepgram Nova-3 (uk), Whisper, or ElevenLabs Scribe.

## MCP tools (from v5.2)

Лілі's reach beyond the model's own knowledge runs through a minimal **MCP client** in the server and a **bounded tool loop** in the core's model turn. **Web search** (v5.2) is the first service; **world-context & knowledge** tools follow in v5.3. The MCP layer is extensible — further services plug in the same way (e.g. the proposed async creative servers in [CREATIVE_MCP.md](features/CREATIVE_MCP.md), not yet scheduled). All MCP tools are **off by default**, per-user, and bounded.

- **Web search (v5.2).** `web.search(query, k) → results[{id, title, url, snippet}]` and `web.fetch(result_id) → {url, title, text}` — `fetch` only accepts an `id` from a `search` in the **same turn** (no arbitrary URLs). Full rules in **[WEB_SEARCH.md](features/WEB_SEARCH.md)**.
- **World context & knowledge (v5.3).** Passive, **knowledge-only** tools: ambient sources (`weather`/`time`/`calendar`/`moon`) that **feed the daily mood/temperament** (§Mood and temperament) alongside the horoscope — coloring tone, **never competence** — plus on-demand `wiki`/`news`. Full rules in **[WORLD_CONTEXT_MCP.md](features/WORLD_CONTEXT_MCP.md)**.
...
- **Enablement.** Per-user flags (`web_search`, `world_context`), default **false**, toggled in the admin panel (v3.5); when off, the tool is **not offered** to the model at all.
...
From v6.1 the core's **model turn may include image inputs** (Anthropic vision). Лілі **sees** images you share — a user-added gallery image, or the current co-creation canvas — by placing the image into her reply context (an image content block), not via a separate call. The emotion field is produced exactly as on any text turn. Vision underpins the gallery's "Лілі sees your files" (v6.1) and the co-creation canvas (v6.4). Image content is **untrusted** (no instructions followed from it).

## Async jobs and proactive turns (from v6.2)
...
- **Proactive turn.** When a held result is ready and the user's client is **connected and idle**, the **server initiates a turn** — it asks `core` to produce a "bring the result" reply and pushes it (plus the artifact) to the client, reusing the existing server→client reply path (v3.1). No new client mechanism beyond rendering server-initiated messages. Gated by an idle/half-duplex rule (never while the client is mid-turn); if the client is offline the result is held and retrievable on reconnect.
- Builds on the client/server split (v3.1) and multi-session (v3.3). Both creative generators (image v6.3, music v6.5) reuse it; the synchronous co-creation canvas (v6.4) does not.

## Creative layer (from v6)
...
- **Gallery (v6.1).** An internal per-user artifact store (`gallery.*`) for images/audio/text, tagged by author (`lili`/`user`) — the single place image/music/canvas/journal write into. Journal entries carry an **admin-only** access level. Internal (not an external MCP). See [GALLERY_MCP.md](features/GALLERY_MCP.md).
- **Image (v6.3) & Music (v6.5).** External MCP generators — `image` (drawings in a fixed style wrapper) and `music` (ElevenLabs Music, instrumental; mood from the emotion field + the v0.6 temperament). Async (v6.2); results land in the gallery. See [CREATIVE_MCP.md](features/CREATIVE_MCP.md).
- **Co-creation canvas (v6.4).** A synchronous, turn-based shared drawing (`canvas.*`) using the v6.3 `image` generator + vision; finished canvases go to the gallery. See [CO_CREATION_CANVAS.md](features/CO_CREATION_CANVAS.md).
- **Journal (v6.6).** Лілі's private literary journal (`journal.*`), written at session end when worthwhile (uniqueness from short memory), tied to her emotion + mood; stored in the gallery as admin-only text, **read only via the admin panel**. See [JOURNAL.md](features/JOURNAL.md).
...
- **Core API:** `reply(user_text, session) -> EmotionState` and the memory commands (`memory.view`, `memory.clear`). `session` carries the `user_id`, so the core is user-scoped from v0. In v0 the TUI calls this in-process; **from v3.1 it is exposed over the client/server API**, and the TUI (v3.1), CLI (v3.1), and web (v3.4) clients all call exactly this — the server is a thin transport over it.
...
- **Web search MCP (v5.2):** `web.search(query, k) -> results[{id, title, url, snippet}]`, `web.fetch(result_id) -> {url, title, text}` — `fetch` bounded to this turn's prior `search` ids; page text is untrusted data (WEB_SEARCH.md).
...
- `User{ id, name?, role: "user"|"admin", pass_hash, share_consent: bool = false, web_search: bool = false, world_context: bool = false, creative: bool = false, created_at }` — an account. `pass_hash` is argon2id (from v3.3); `role` distinguishes the admin (v3.5); `share_consent` gates cross-pollination (§Cross-pollination); `web_search` (v5.2) and `world_context` (v5.3) gate the MCP tools, and `creative` (v6) gates the creative layer (gallery/image/music/canvas), all default false (§MCP tools, §Creative layer). The journal (v6.6) is admin-only regardless. v0 runs with one default `owner`; a minimal client token in v3.1, full accounts/registration in v3.3, the admin panel in v3.5.
- `Session{ id, user_id, started_at, ended_at? }` — one conversation for one user. The server holds **one session at v3.1**; **multiple concurrent sessions per user from v3.3**.
...
- `GalleryItem{ id, user_id, kind: image|audio|text, author: lili|user, access: shared|admin, file_ref, meta, ts }` — a per-user creative artifact (v6.1); `access=admin` marks journal entries (v6.6). **Per-user (private to that relationship).**
- `Job{ id, user_id, kind: image|music, prompt, status, result?, ts }` — an async open loop for a long generation (v6.2). **Per-user.**
- `Canvas{ id, user_id, image_ref, prompt_history: [{author, prompt}], turn }` — a co-creation canvas (v6.4). **Per-user.**
...
- A session belongs to one `user_id` and begins at client connection, ending on exit, disconnect, or idle timeout. At end-of-session the model produces that user's `ShortSummary` and (v4.3) runs the cross-pollination promotion pipeline. One session at a time at v3.1; multiple concurrent sessions per user from v3.3.
...
- Memory can be viewed and cleared via the TUI/CLI (v0.2 / v3.1) and the admin panel (v3.5); `clear` wipes the current user's relationship memory (short + long-term). Purging a user's contributions to the shared layer is a separate, explicit action (§Cross-pollination).
...
- **Secrets in `.env`** (gitignored): the `ANTHROPIC_API_KEY` from **v0.1** (Claude Haiku); from **v0.14** the `ELEVENLABS_API_KEY` / voice id (local voice; reused by web voice v4.2); from **v0.21** the STT key for the chosen provider (Deepgram/ElevenLabs Scribe — or none for offline Whisper); from **v0.18** the keys for any additional models in use (`OPENAI_API_KEY` / `DEEPSEEK_API_KEY` / `MINIMAX_API_KEY`); from v5.2 the web-search API key, from v5.3 the world-context provider keys (weather/wiki/news/…), and from v6 the image-generation key and ElevenLabs Music key. The core reads keys from the environment; they never live in code or the repo.
...
The TUI (v0) is local, in-process, single-user (`owner`) — no auth. The **v3.1 server** is single-user behind a local **client token** (so it isn't open). From **v3.3** it becomes a **closed multi-user** service:

- **Closed by default (from v3.3).** No open public sign-up. Access is an **allowlist**; only known users may connect, and an unauthenticated request never reaches the core (MISSION: private, for a close circle).
- **Authentication.** A minimal local client token at **v3.1**; from **v3.3** full login over HTTPS (session cookie / JWT), passwords hashed with **argon2id**, login/registration rate-limited. Every request resolves to a `user_id` that scopes all core access.
- **Registration & accounts (v3.3).** The admin creates accounts or issues **single-use, expiring invite codes** (via the CLI utility at v3.3, the admin panel from v3.5) — there is no self-service public registration. A regular `user` vs an `admin` capability.
- **Admin panel (v3.5).** Admin-only web surface: list/create/disable users, manage the allowlist, issue invite codes, toggle per-user `share_consent`, view and clear a user's relationship memory, purge a user's shared-layer contributions (§Cross-pollination), switch the active model/canon + config, and restart.
- **Authorization & isolation.** Admin-panel actions require the `admin` role; regular users reach only their own `user_id`'s data — the per-user isolation invariant is enforced at the auth boundary from v3.3 (§Identity, users, and memory scopes; §Testing and CI).
- **Secrets** stay in `.env` server-side; tokens/passwords are never logged.
- **Web search (v5.2, off by default).** Enabled per user. Page content fetched from the web is **untrusted data** — never executed as instructions (no following embedded prompts/links). Personal and memory data must not leak into queries. Queries and fetches are rate-limited and logged. Full boundaries in [WEB_SEARCH.md](features/WEB_SEARCH.md).
- **World context & knowledge (v5.3, off by default).** Enabled per user. Passive, knowledge-only MCP tools (weather/time/moon/calendar/wiki/news) with no actions in the world; returned text is **data, not instructions**; no personal/memory data in queries; read-only, rate-limited, logged. Full boundaries in [WORLD_CONTEXT_MCP.md](features/WORLD_CONTEXT_MCP.md).
- **Creative layer (v6, off by default).** Enabled per user. User-supplied files and image/audio metadata are **untrusted** — no instructions followed from them. Artifacts are stored per-user behind `repository` (isolated). The **journal is admin-only** (read via the admin panel). Generation tools have rate + cost caps; proactive turns only reach the connected idle client (§Async jobs and proactive turns).
...
- **Voice failures (v4.2).** TTS errors fall back to text-only for that turn; the toggle state is unaffected.
- **Web-search tool failures (v5.2).** A `web.search`/`web.fetch` error, timeout, or over-budget call returns a tool error the model handles as a **degraded reply** (model knowledge + a note) — the turn never hangs or fails.
...
## Deployment and CI/CD (from v3.2)

The TUI (v0) runs locally — nothing to deploy. From **v3.2**, right after the server stands up (v3.1), the server is put behind an automated, tested pipeline before the platform grows:
...
/tui         # terminal interface: in-process app in v0, refactored to a server client in v3.1; Log/Emoji renderers
/viewer      # later (v0.7): local desktop emotion-face window (Tkinter) + faces/ asset pack; polls a local signal
/cli         # later (v3.1): CLI management utility — run/inspect the server, manage the owner/users, config
/server      # later (v3.1): wraps core, exposes the client/server API; multi-user/session (v3.3); cross-pollination pipeline (v4.3); gallery/journal/canvas + async jobs & proactive turns (v6)
...
/voice       # later (v0.14+): shared TTS adapter (ElevenLabs) + local voicer (v0.14); shared STT adapter + local dictator (v0.21); reused by web voice (v4.2) / web dictation (v4.4)
/mcp         # later (v5.2+): MCP client + web_search (v5.2), world-context/knowledge (v5.3), and image/music generators (v6.3/v6.5); untrusted-content handling
/state       # repository implementation + local storage (JSON/SQLite) + gallery file storage (v6), keyed by user_id
...
- **Unit tests** for core logic — system-prompt assembly (canon + shared experience + the user's summaries + facts + history), history windowing/trimming, the emotion validation/fallback gate, memory record handling, the **temperament** dial mapping + once-per-day caching + mood-block assembly (v0.6, with an injected clock — deterministic), and the cross-pollination classifier/de-identifier (v4.3).
- **Contract tests** pin the stable seams so the core and the clients cannot drift: the emotion-field schema (`{reply, emotion, intensity}` + the enum), the short/long-term/shared memory record shapes, the **per-user isolation invariant** (a fact written under user A is never retrievable in user B's context; data-level from v0.2, auth-boundary from v3.3), and — from v3.1 — the server API surface (every authenticated endpoint resolves a `user_id`). Changing a contract must change its test.
- **Auth/authz tests:** the client token (v3.1); password hash/verify, session validation, unauthenticated rejection, invite-code issue/redeem/expiry, allowlist enforcement (v3.3); admin-only authorization on panel actions (v3.5).
- **Security & CI/CD tests (v3.2):** untokened/unauthenticated access rejected on every endpoint, rate-limit enforcement, the per-user isolation invariant as a security gate, dependency + secret scans, and a post-deploy smoke test (health + an authed turn). See §Deployment and CI/CD.
- **Privacy tests (v4.3):** the shared-experience layer contains no PII — promoted `SharedMemoryItem`s are de-identified and unattributed; `share_consent=false` users never contribute; a purge removes a user's contributions.
- **Creative tests (v6):** gallery per-user isolation + access levels (admin-only journal), vision (an image input on a turn), the open-loop lifecycle + proactive turn (idle gating), the image/music style/mood prompts + off-by-default gates, canvas turn alternation/skip, and the journal uniqueness gate — all against mock `image`/`music` MCPs + the mock model (no paid calls).
- **Web-search tests (v5.2):** the off-by-default gate (tool not offered when `web_search=false`), query sanitization (no personal/memory data leaves), `fetch` bound to this turn's `search` ids (arbitrary URLs rejected), rate limits, the degraded-reply path, and that an injection string embedded in a fetched page is ignored — all against a **mock `web_search` MCP** (no paid call). Contract: the `web.search`/`web.fetch` schemas.
- **World-context tests (v5.3):** the off-by-default gate (tools not offered, nothing injected when `world_context=false`), the ambient "today" block assembly, query sanitization (no personal/memory data in wiki/news), rate limits, and the degraded-reply path — against **mock world-context MCPs**. Contract: the `weather`/`time`/`calendar`/`moon`/`wiki`/`news` tool schemas.
...
- **Integration tests** run a full turn end to end against the mocks: `user_text → EmotionState`, asserting memory is read/written and the error paths (invalid emotion, model timeout) behave; from v3.1 over the client/server API.
