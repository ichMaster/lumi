...
- **Short memory** — at the end of a session the model compresses it (one call) into **two tiers**: a **detailed** summary and a one-line **gist** (`ShortSummary{user_id, session_id, summary, gist, ts}`). From **v0.9** the prompt injects the **last N conversations in detail** (`summary`) plus **all conversations in the last D local days as gists** — the recent N not repeated, dated, bounded by the day window (no row cap) — so Лілі recalls recent chats vividly and the past few days at a glance. (Before v0.9 there is only the detailed `summary`; old records load with an empty `gist`.)
