...
- **Voice.** A shared **ElevenLabs TTS adapter** voices Лілі's replies — a **local voicer** (v0.16) then web voice output (v2.2) — and a shared **STT adapter** hears the user — a **local dictator** (v0.17) then web dictation (v2.4). Both local pieces are separate console processes coupled to the core by files.
...
- **Web dictation (STT, v2.4).** The same STT adapter, now in the browser: microphone → Ukrainian recognition → text in the input field. The web sibling of the v0.17 local dictator. Provider options: Deepgram Nova-3 (uk), Whisper, or ElevenLabs Scribe.
...
/voice       # later (v0.16+): shared TTS adapter (ElevenLabs) + local voicer (v0.16); shared STT adapter + local dictator (v0.17); reused by web voice (v2.2) / web dictation (v2.4)
